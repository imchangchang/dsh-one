...
export const SLOT_DEPENDENCIES = [
  ...
  },
  {
    ...
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:1500',
  ...
  },
  {
    ...
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:1435-1437',
...
export const ROOT_HOOK_DEPENDENCIES = [
  ...
  },
  {
    ...
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:368',
  ...
  },
  {
    ...
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:370',
    expect: 'packages/client/ui-session（provideRoot 下发点）',
  },
  {
    name: 'workspaces',
    ...
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:369',
...
export const IDENTIFIER_DEPENDENCIES = [
  ...
  },
  {
    ...
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:330',
  },
  {
    names: ['sessionIds'],
    scope: ['@deepseek-ai/dsh-client-ui-workspace'],
    why: '工作区视图的成员会话 id 列（自有树按它分组）',
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:337',
  },
  {
    names: ['workspaceId'],
    scope: ['@deepseek-ai/dsh-client-ui-workspace'],
    why: '工作区 id 字段（分组行与动作按它寻址）',
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:335',
  },
  {
    names: ['createdAt'],
    scope: ['@deepseek-ai/dsh-client-ui-workspace'],
    why: '工作区创建时间字段（排序与相对时间显示）',
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:338',
  },
  {
    names: ['byId'],
    scope: ['@deepseek-ai/dsh-api-session-controller'],
    why: 'sessions 服务快照的 id → 会话摘要表（自有行标题/cwd/未读都取自它）',
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:1054,1076,1417',
  },
  {
    names: ['pendingInteractions'],
    scope: ['@deepseek-ai/dsh-client-ui-session'],
    why: 'sessionPendingInteraction hook 的源（会话级等待态 Map）',
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:370（PendingMap）',
  },
]

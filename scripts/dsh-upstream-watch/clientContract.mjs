...
export const SLOT_DEPENDENCIES = [
  ...
  },
  {
    ...
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:316,319',
  ...
  },
  {
    ...
    where: 'src/ui/assembly/shell/workspaceTreePlugin.ts:200-202',
...
export const ROOT_HOOK_DEPENDENCIES = [
  ...
  },
  {
    ...
    where: 'src/ui/assembly/shell/workspaceTree/types.ts:47',
  ...
  },
  {
    ...
    where: 'src/ui/assembly/shell/workspaceTree/types.ts:49',
    expect: 'packages/client/ui-session（provideRoot 下发点）',
  },
  {
    name: 'workspaces',
    ...
    where: 'src/ui/assembly/shell/workspaceTree/types.ts:48',
...
export const IDENTIFIER_DEPENDENCIES = [
  {
    ...
    where: 'src/ui/assembly/shell/composerClearPlugin.ts:106（attachmentIdsOf，两代都取）',
  },
  {
    names: ['addAttachments', 'addImages'],
    scope: ['@deepseek-ai/dsh-client-ui-conversation'],
    why: '还回待发附件的动作（0.1.6 由 `addImages` 改名成 `addAttachments`）',
    where: 'src/ui/assembly/shell/composerClearPlugin.ts:118-122',
  },
  {
    names: ['removeAttachment', 'removeImage'],
    scope: ['@deepseek-ai/dsh-client-ui-conversation'],
    why: '移除待发附件的动作（0.1.6 由 `removeImage` 改名成 `removeAttachment`）',
    where: 'src/ui/assembly/shell/composerClearPlugin.ts:110-116',
  },
  {
    names: ['setDraft'],
    scope: ['@deepseek-ai/dsh-client-ui-conversation'],
    why: '撤销清空时写回正文的输入面动作',
    where: 'src/ui/assembly/shell/composerClearPlugin.ts:87',
  },
  {
    names: ['draftRev'],
    scope: ['@deepseek-ai/dsh-client-ui-conversation'],
    why: '插入引用 chip 时的 span CAS 版本号（每次现读）',
    where: 'src/ui/assembly/shell/composerClearPlugin.ts:137,152',
  },
  {
    names: ['insertReference'],
    scope: ['@deepseek-ai/dsh-client-ui-conversation'],
    why: '撤销清空时把引用 chip 插回去的输入面动作',
    where: 'src/ui/assembly/shell/composerClearPlugin.ts:138,148',
  },
  {
    names: ['activePanelId'],
  ...
  },
  {
    ...
    where: 'src/ui/assembly/shell/workspaceTree/types.ts:9',
  },
  {
    names: ['sessionIds'],
    scope: ['@deepseek-ai/dsh-client-ui-workspace'],
    why: '工作区视图的成员会话 id 列（自有树按它分组）',
    where: 'src/ui/assembly/shell/workspaceTree/types.ts:18',
  },
  {
    names: ['workspaceId'],
    scope: ['@deepseek-ai/dsh-client-ui-workspace'],
    why: '工作区 id 字段（分组行与动作按它寻址）',
    where: 'src/ui/assembly/shell/workspaceTree/types.ts:15',
  },
  {
    names: ['createdAt'],
    scope: ['@deepseek-ai/dsh-client-ui-workspace'],
    why: '工作区创建时间字段（排序与相对时间显示）',
    where: 'src/ui/assembly/shell/workspaceTree/types.ts:19',
  },
  {
    names: ['byId'],
    scope: ['@deepseek-ai/dsh-api-session-controller'],
    why: 'sessions 服务快照的 id → 会话摘要表（自有行标题/cwd/未读都取自它）',
    where: 'src/ui/assembly/shell/workspaceTree/tree.ts:257,279、workspaceTreePlugin.ts:182',
  },
  {
    names: ['pendingInteractions'],
    scope: ['@deepseek-ai/dsh-client-ui-session'],
    why: 'sessionPendingInteraction hook 的源（会话级等待态 Map）',
    where: 'src/ui/assembly/shell/workspaceTree/types.ts:23（PendingMap）',
  },
]

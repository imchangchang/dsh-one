...
# （如 develop/dsh-web-alignment）。该分支必须已被某个 worktree 检出：rebase 目标、
# 合并执行位置、复测和 dist 重建都在那个 worktree 里进行，主工作区不受影响。
set -euo pipefail
...
# 没为集成线单独开 worktree 时它就是主工作区（默认 main 的老行为）。
...
# i18n 合入门禁：待合入分支相对集成线的新增行若漏同步 i18n（宿主/ webview / manifest /
...
# 同样的冲突逐提交重复出现；保留 merge 后集成线没新增提交时等价空操作。
...
if ! GIT_EDITOR=true git -C "$WT" rebase --rebase-merges "$TARGET"; then
  cat >&2 <<EOF
rebase 有冲突。进入 $WT 解决：
  cd $WT

...
3. **启动服务**：先探测配置端口（默认 3080）——POST `/api/host.describe` 并校验回包 `rpcId` 一致，确认是 dsh 就**收养**（只连接，永不 kill）；否则用 `node lib/bin.js web --host 127.0.0.1 --port <端口>` 自己 spawn 一个（直跑 node + bin.js，绕开 shim/shell 问题）。就绪需要双重确认：先解析 stdout 的 `dsh web: http://127.0.0.1:<端口>` 行，再做一次 `host.describe` 身份确认。
...
| `dshOne.pinnedVersion` | `string` | `""` | 非空则钉死该版本，禁用自动更新 |
...
- 插件**只会终止自己 spawn 的 dsh 进程**；收养的实例在任何路径下都不会被 kill。
- 关闭 VSCode 时同步发送 SIGTERM（Windows 用 `taskkill /T /F`），3 秒后由独立 reaper 进程兜底强杀。
...
- **多窗口**：每个 VSCode 窗口各自管理服务；端口被占用时靠收养机制共享已有实例，端口为 0 时各窗口各自起实例。

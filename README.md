...
`@next` resolves to `0.1.5-rc.3` today, which is the same version the untagged `npm i -g @deepseek-ai/dsh` (npm `latest`) installs — **both tags point at the verified 0.1.5 line**, so either command lands on a supported version.
...
- **VS Code**: `^1.96.0`.
- **dsh**: installed by you via npm (Node `^22.19` or `>= 24`). The **0.1.5 line** is verified end to end, and it is the line both npm tags point at today (`latest` = `next` = `0.1.5-rc.3`), so the plain install and the `@next` below both land on a verified version.
- **Platforms**: Windows / macOS / Linux.

### dsh version tracking

A scheduled GitHub Action ([dsh-upstream-watch](.github/workflows/dsh-upstream-watch.yml)) checks for new [dsh releases](https://github.com/deepseek-ai/deepseek-harness/releases) daily; for each new version it runs the automated probe suite (24 checks: the wire protocol, the front-end artifacts the gateway serves — boot contract, combo endpoint, Origin fence —, the client contract surface the assembled UI depends on, and the installed official artifacts: the internal identifiers inside the official packages plus the export names of the 26 official icons we take, a family that broke every one of our plugins once when upstream renamed it) and files an `upstream-watch` issue with the results. The last two badges above show the latest upstream release and the latest probe outcome; the full test list, per-version readings and known failures live in [docs/dsh-compat-checklist.md](docs/dsh-compat-checklist.md).

**dsh versions.** Measured per version with a whole round of `npm run verify:lab`. The three groups are kept apart on purpose:

| dsh version | Status |
|---|---|
| `0.1.6-alpha.1` | **verified** — 69 suites, 3620 assertions, zero failures (2026-09-22) |
| `0.1.6-alpha.2` | verified — four drifts were found and fixed here (none of them visible to the probes, which only read bytes): the client started adopting the plugin roster the gateway pushes over `/plugins/events`, which reinstated the plugins we block and removed our own (the whole page rendered blank); the session list stopped carrying `current`, which left the sidebar tree with workspaces but no sessions (and the git card without a working directory); the session service dropped `open` / `select` / `clear`, which made clicking a session do nothing; and the `completed` flag moved off the session row into the `sessionStatus` hook, which lost the “finished, not opened yet” dot. One further drift (fixed in #225) broke the sidebar panel outright: a conversation-area card began waiting for a service the sidebar tree does not provide, so the panel showed the official boot-audit failure instead of the session tree. (Drift record for the earlier `0.1.6-alpha.1` round: the `details` slot was renamed to `rightbar`, new root-level hooks appeared, and the composer's `imageIds` / `addImages` became `attachmentIds` / `addAttachments`.) |
| `0.1.5-rc.3` | **verified** — 73 suites, 3681 assertions, zero failures (2026-09-23, #238; 73 was the suite count of that round, before F-73 landed in #243). **This is what both npm tags install today** (`latest` = `next` = 0.1.5-rc.3), and what the `@next` in the quick start resolves to |
| `0.1.5-rc.2` | **verified** — 73 suites, 3681 assertions, zero failures (2026-09-23, #238; 73 was the suite count of that round, before F-73 landed in #243); it is the floor of the version gate. Earlier the same line read “verified broken” with two failures, both of which turned out to be judgements pinned to 0.1.6-only shapes and were fixed in #238: the composer's ＋ is present in that generation too — it simply carries a different official label («指令» / «Commands» instead of “Add files or run commands”, that generation has no `pickFiles`) — and that generation reports a failing entry as `failed to … loader entry …` rather than the boot audit, which the startup self-heal now recognises (with one documented consequence: the per-audit entry count does not exist there, so the “too many entries at once” guard cannot apply). One limitation of that generation itself: archiving a session has **no undo** there — the official artifacts carry no unarchive carrier and no `unarchiveSession` method at all, so the archived-session section our tree adds for 0.1.7 deliberately does not appear (an entry without an action behind it would be a dead button); F-72 judges that negative invariant and records it |
| `0.1.7-alpha.2` | **verified broken** — 48 of 74 suites pass / 26 fail (2026-09-24, exclusive window). Both blockers that made the whole round fail are fixed. **#236**: the official `@deepseek-ai/dsh-client-ui-primitives` renamed its whole icon set from size suffixes to weight suffixes (`IconCloseFill14` → `IconCloseFillMedium` / `IconCloseFillRegular`, and 25 more we import); a name that is gone resolves to `undefined` instead of failing to load, so our sidebar tree's slot entry crashed at render (`slot entry crashed in 'sidebar.workspaces': Error: Minified React error #130`). **#243**: the roster baseline parameter was built by comma-joining the per-batch revs, and 0.1.7 splits the application batch in two so a rev now really contains a comma — the parameter came out truncated, the mirror read that as “the whole roster changed”, and tearing our own frame plugin down took the `root` slot registration with it and blanked the page. The remaining 26 failures are classified entry by entry in #244: 21 are judgements or fixtures pinned to 0.1.6 shapes (that generation redrew its icons, reworded entries and changed the slot shapes; the features themselves work), **none** is confirmed product breakage on 0.1.7, and 5 are not yet classified — F-71 among them is the suite's own click timeout, so it is an unstable test rather than a broken product. The same release also made `__DSH_BOOT__.batches[].url` relative (`plugins/??…` instead of `/plugins/??…`), which broke the mirror's combo fetch before anything could run at all — that one is fixed too. **Not a supported version.** |
...
| every other version | **not tested** — including a `0.1.6` release (if one ships) and anything at or above `0.2.0` |

**Version gate.** The assembled chat expects dsh `[0.1.5-rc.2, 0.2.0)`: the floor is the oldest version that passes a whole round, and nothing above the ceiling has been verified. The gate never blocks — a version outside the range gets an info banner at the top of the panel, and the banner carries the install command to run, not just the range: `npm install -g @deepseek-ai/dsh@0.1.5-rc.3`, which is the zero-failure round **and** the version both npm tags point at today (so following the quick start or the one-line script below also lands on it). `alpha` = `0.1.7-alpha.2` is inside the range but measured broken — do not install it. One known limitation: the gate is a whole-range check, so a version inside the range that is measured broken — `0.1.7-alpha.2` right now — passes silently.

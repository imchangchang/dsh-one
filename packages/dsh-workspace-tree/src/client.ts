/**
 * `@dsh-one/dsh-workspace-tree` 的浏览器侧入口。
 *
 * 包清单的 `exports["./client"]` 指向本文件，`build.mjs` 以它为 entry 打官方 combo
 * 格式的 `lib/client.js`。插件本体就在本包里（`workspaceTreePlugin.ts` 是注册与组合，
 * `workspaceTree/` 下是行、工具栏、分组条、抽屉、对话框、样式等分件），本文件只把
 * 它的 `apply` / `inject` 转出去——**一个包 = 完整插件**，源与产物都在包内（#94）。
 *
 ...
 */
export { apply, inject } from './workspaceTreeEntry.ts'

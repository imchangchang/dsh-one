 ...
 * 官方客户端**没有界面**——它落到会话对象的 `lastAgentError` 上（0.1.6-alpha.1 全仓
 * 只有一个写入方、没有读取方，见 `workspaceTreePlugin.ts` 里 `SessionSnapshotFace` 的
 * 说明），用户当场看到的只有之后发消息时那条原始吐司
 * （`resume failed for session …: SessionAlreadyOwnedError: …`），既看不出原因也没有
 * 下一步。侧栏树是用户点击的地方，在这里给一条能行动的提示。
 *
 * 为什么走模块级信号而不是 props：订阅的安装点在插件 `apply`（拿得到官方客户端服务
 * `sessions`，判据是那条会话官方快照上的失败字段），而**文案的翻译**要做在组件里
 * （`tr` 是组件那一层的东西，官方没有给插件层的同步取词口）。两者之间就用这条模块级
 * 信号连起来——与 `flash.ts`（飘提示）、`selection.ts`（选择态入口信号）同一处置。
 *

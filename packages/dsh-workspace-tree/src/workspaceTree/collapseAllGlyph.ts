...
/**
 * 把这两枚图标放大到与同一排官方图标一样重的倍数（#141）。
 *
 * 起因：用户实测报「这一枚比同一排其它几枚小一圈」。原因是官方那几枚几乎画满自己的
 * 16 格画布，而我们的方框在 16 格坐标里只占 2.5～13.5（11 格，约 69%）——渲染尺寸
 * 同为 16×16，画出来的东西却小。
 *
 * 1.25 是按 #141 在装配页上量出来的（同一排五枚图标的实际绘制范围，单位 CSS px，
 * 官方那四枚的读数见装配实验室的 F-40）：
 *
 * | 图标 | 绘制范围（宽×高） | 最小边 |
 * | --- | --- | --- |
 * | 搜索 | 14.00×14.00 | 14.00 |
 * | 添加工作区 | 15.17×12.46 | 12.46 |
 * | 设置 | 14.95×15.36 | 14.95 |
 * | 多选 | 14.32×13.54 | 13.54 |
 * | 本枚（放大前） | 11.00×11.00 | 11.00 |
 *
 * 四枚官方图标最小边的中位数是 13.77。放大 1.25 后本枚的绘制范围是 13.75×13.75，
 * 与那个中位数差 0.02px（判据是「差 ≤ 1px」，不写死像素）；顺带一提面积 189 也落在
 * 四枚中位数 195 的 97%，两条量同时对上，所以取这个倍数。方框的线宽在屏幕上从
 * 1.20px 变成 1.50px——同一排最粗的一枚官方图标（多选）量出来是 1.46px，不会显得
 * 比它们粗。
 */
export const COLLAPSE_ALL_GLYPH_SCALE = 1.25

/**
 * 上面那个倍数的 transform 原文：前后两条 `translate` 把缩放的原点挪到图标中心
 * （16 格坐标的 `8 8`，也是方框的中心），所以放大只让方框向外长、中心不动。
 *
 * 为什么走 transform 而不是放大 `svg` 的尺寸或改 `d`：`svg` 的 16×16 渲染尺寸与
 * 26×26 的按钮、图标居中都是既有断言（F-25 / F-23）钉住的；`d` 则要与旧侧栏那两条
 * 逐字相同（`test/collapseAllGlyph.test.ts`）。挂在 path 上只影响画出来的大小，这两样
 * 都不用动。
 */
export const COLLAPSE_ALL_GLYPH_TRANSFORM = `translate(8 8) scale(${String(COLLAPSE_ALL_GLYPH_SCALE)}) translate(-8 -8)`

...
//   （`SCALE_TIERS.compact.rowPaddingInline`，与父项**同一个值**）——即**不再额外缩进**（#174）：
//   子项文字与父项文字落在同一条竖线上（差值 **0**）。旧侧栏（正本）那条关系量实测是 **+14px**
...
//   见 `test/legacy-sidebar/` 的台账）。历史：#143 收到 0、#167 取回旧侧栏那一档（一个图标槽）、
//   #171 取那一档的一半（半个图标槽 = 7px）而用户看着**仍觉得偏大**——真因是子项没有图标，文字
//   左边那段空白大半来自 rows.ts 补的**空图标槽**（14 + 项内间隙 6 = 20px），那 7px 只是最后一
//   小步，所以 #174 收到底取 **0**（空图标槽**保留**：去掉它子项文字会跑到父项**图标**那一列）。
//   取值出在官方紧凑档，算式与出处写在同一条规则上方。
...
export const SCALE_EXEMPT: readonly { selector: string; reason: string }[] = [
  ...
  // 二级菜单项的缩进（#126 立、#143 / #167 / #171 / #174 各重定过一回）：**原先那条豁免随 #143
  // 退场**——#174 起取值 = 紧凑档的项内边距 7px（`SCALE_TIERS.compact.rowPaddingInline`，与父项
  // 同一个值，即不再额外缩进），引用的就是档位表里的那一个量，不需要例外登记。
...
export const CSS =
  ...
  // #174 收到 0）。官方 Menu 的项是一条「图标槽 + 文字 + 勾」的流水线，官方没有「子项缩进 /
  // 层级」这个口（前三层都没有：
  ...
  // submenuChild），因此这条关系落在**官方项那个 `<button role="menuitem">`（官方给每个菜单项
  // 打的语义属性，同一份源码）的左内边距**上：用 :has() 从我们自己的标记类去选它的祖先项，不碰
  // 任何官方哈希类名。
  //
  // 取值 = **紧凑档的一个项内边距 7px**（`SCALE_TIERS.compact.rowPaddingInline`，官方
  // `._item_1nxmc_92{padding:3px 7px}`）——与父项**完全一样**，也就是**不再额外缩进**：子项整行
  // 比父项不多让开一点。算式（每一项都出在官方紧凑档，都相对项盒左缘）：
  //   子项文字左缘 = 项内边距 7 + 空图标槽 14 + 项内间隙 6 = 27
  //   父项文字左缘 = 项内边距 7 + 图标槽 14 + 项内间隙 6 = 27
  //   → 差值 **0**：子项文字与父项文字落在同一条竖线上。这一句因此与官方项自己的左内边距同值
  //   （等于不写）；仍然显式留着，是因为这条规则是「二级项文字列」这件事在源码里的唯一落点
  //   （说明、套件与报告都指着它），把「0 额外缩进」这个取值写在明面上比留一条空规则好读。
  // 为什么取 0 而不是旧侧栏那一档：旧侧栏（正本）的同一条关系实测就是 **+14px**（父项
  ...
  // `submenu-text-vs-parent-text`）——#167 先照那一档取回一个图标槽（+14），#171 取它的一半
  // （+7 = 一个项内边距），用户看过**仍觉得偏大**：子项没有图标，它文字左边那段空白里**大部分
  // 是空图标槽**（14 + 项内间隙 6 = 20px），多让开的那半格只是最后一小步。#174 收到底：0。
  // 层级靠什么表达：父项右端的 ▸/▾ 指示 + 子项自己的图标槽（标签组色块就画在那一列）——
  // 见 rows.ts 里 submenuParent / submenuChild 的说明。
  // 三版历史：#126 这条写 27px（= 项内边距 + 图标槽 + 间隙，子项文字比父项深 20px），用户实测
  // 报「太深」；#143 收到 7px（与父项同列，深 0）——用户看下来「层级没了」；#167 拍板「留一点」、
  // 取回旧侧栏那一档（一个图标槽 14px）；#171 再收一半到 7px，用户看仍偏大；#174 回到 0，并明确
  // **保留**空图标槽（去掉它子项文字会跑到父项**图标**那一列、比父项文字还靠左，是另一个形态）。
  // 这条关系量由 `test/assembly-lab/` 的 F-32 在真装配页上钉住（两个菜单各一遍，期望值从
  // `SCALE_TIERS` 读），回归钉子：**不许深过旧侧栏那一档**（一个图标槽 14px）。

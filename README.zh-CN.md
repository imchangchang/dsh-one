...
| `0.1.5-rc.3` | **实测通过**——70 项全绿、3650 条断言零红（2026-09-23 实测，#238）。**它就是 npm `latest`（= `next`）今天指的版本**，也是快速开始的 `@next` 装到的那一版 |
| `0.1.5-rc.2` | **实测通过**——70 项全绿、3650 条断言零红（2026-09-23 实测，#238）；它是**版本门的下界**。这一版早先的「实测不通」是两条判据钉了 0.1.6 才有的形状，已在 #238 逐条定性后修掉：那一代的 composer ＋ 也在场、只是官方给它换了标签（「指令」/「Commands」，那一代没有 `pickFiles`）；那一代报「某条目起不来」用的是 `failed to … loader entry …` 而不是启动审计那一句，启动自愈现在两种都认（代价如实写进声明：那一代报不出条数，所以「一次点名太多就不摘」这条规模闸在那一代判不出来） |

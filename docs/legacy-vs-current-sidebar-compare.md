...
| 移到分组…（就地展开的二级项：各组 / 不归入 / 新建） | `sessionsWebview.ts:3337-3341`、`3406-3456` | `rows.ts:1054-1060`、`tree.ts:1041-1064` | 不同（子项内容：旧含三个内置组 + 「不归入分组」+「新建分组…」；现无内置组，含自建组 + 「不归入标签组」+「新建标签组…」。出现条件：旧**恒出现**，现只有这一行有标签组入口时出现——单列表形态不出现） |
...
### A3.3 标签组 pill 菜单（旧 8 项 / 现 8 项）

| 菜单项 | 旧（`文件:行`） | 现（`文件:行`） | 判断 |
| --- | --- | --- | --- |
| 怎么打开 | **右键 pill**：`sessionsWebview.ts:2002-2010` | 组头右侧 **⋯ 按钮**：`tagGroups.ts:262-276`、`373-393` | 不同 |
| 标题行「分组: X」 | `sessionsWebview.ts:2113` | `tagGroups.ts:116` | 一致 |
| 在此分组新建会话 | `sessionsWebview.ts:2116-2124` | `tagGroups.ts:117` + `tree.ts:998-1006` | 不同（未分组桶里这一项不落，因为那一桶开不出会话） |
| 归档 N 个会话（无可归档时禁用 + 提示） | `sessionsWebview.ts:2128-2139` | `tagGroups.ts:118-124` | 一致 |
| 移 N 个会话到回收站（全置顶时禁用） | `sessionsWebview.ts:2140-2153` | `tagGroups.ts:125-131`（无可回收时禁用） | 一致 |
| 移出分组 | `sessionsWebview.ts:2154-2162` | `tagGroups.ts:132` + `tree.ts:1019-1022` | 一致 |
| 重命名分组…（宿主输入框） | `sessionsWebview.ts:2164-2172` | `tagGroups.ts:133` + `tree.ts:1627-1642`（面板内模态） | 不同（弹窗形态） |
| 颜色（二级子菜单 6 色，当前色 ✓） | `sessionsWebview.ts:2174`、`2189-2224` | `tagGroups.ts:134-140`（同一菜单里的**一节**：小标题 + 6 个色块项） | 不同（二级 → 一节） |
| 删除分组（旧仅在非内置组出现） | `sessionsWebview.ts:2175-2185` | `tagGroups.ts:141-147`（危险色；已无内置组概念） | 一致 |
...
| 置顶排序 | 同层最前，**置顶项之间按置顶的先后**；其后还有一层「活跃会话前置」：`pure/sessionTree.ts:287-295`、`337-380` | 同层最前，**置顶项之间保持官方顺序**；没有「活跃前置」这一层：`pure/sessionMarks.ts:135-139`、`tree.ts:524-525`、`pure/sessionTagGroups.ts:365-367` | 不同 |
...
| 抽屉头 | 「‹ 返回」+ 标题（紧跟计数徽标）+ 清空（34×34，危险色）+ 「恢复全部」文本按钮：`sessionsWebview.ts:2772-2800` | 标题 + 计数 + ✕ 关闭：`recycleDrawer.ts:274-287`；清空与全部还原只剩入口行那两枚：`recycleEntry.ts:101-120` | 不同（抽屉头少了两枚动作与「返回」，多了 ✕） |
...
| 回收站行显示 | 状态点 + 图钉 + 标题 + 时间：`sessionsWebview.ts:2824-2863` | 标题 + 时间 + 动作：`recycleDrawer.ts:382-446`（**没有状态点与图钉**） | 缺（回收站行不再显示状态与置顶标记） |
| 回收站行动作 | 只有 ⋯ 菜单（恢复 / 归档） | 行内「还原」按钮 **+** ⋯ 菜单：`recycleDrawer.ts:396-444` | 现在更好 |
...
| 视图选项（按工作区 / 单列表、手动序 / 最近更新） | 无（旧侧栏只有按工作区一种，也没有排序选项） | `toolbar.ts:45-99`、`tree.ts:1458-1461` | 现在更好（新增） |
...
| 拖拽中的源行视觉 | 半透明：`sessionsView.ts:634` | 无（`grep -rn dragging src/ui/assembly/shell/workspaceTree/` 只有行内改名的一段逻辑，没有拖拽样式） | 缺（拖动时源行没有反馈） |
| 工作区分组的拖拽排序 | 有（管理视图里拖组行）：`sessionsWebview.ts:1107-1168`、`sessionsView.ts:983-989` | 无（`pure/workspaceGroups.ts:97` 的 `reorderGroups` 只被旧 store 调用，装配树没有入口） | 缺 |
| 排序口径 | 置顶最前（按置顶顺序）→ **活跃会话前置**（运行中 / 后代在跑 / 未读 / 待交互）→ 其余按最近更新：`pure/sessionTree.ts:337-380` | 置顶最前（置顶之间保持原顺序）→ 官方顺序或「最近更新」：`tree.ts:520-525` | 不同（少了「活跃前置」这一层） |
...
| 回收站入口行 | 行高由 `padding: 7px 4px 7px 14px` + 12px 字撑出（`sessionsView.ts:334-339`）；行内图标按钮 26×26（`341`） | `footer-row-height`：42px / 26px（`sidebarFramePlugin.ts:183`；消费点 `styles.ts:371`）；动作按钮 `icon-button-size`：28px / 26px（`styles.ts:376`） | 不同（官方档 42px 对齐同座位的官方条目） |
...
12. **排序少了「活跃会话前置」这一层**：旧的在每个工作区里先把运行中 / 有后代在跑 / 未读 / 待交互的会话提到前面（`pure/sessionTree.ts:337-380`），现在只有「置顶最前」，其余保持官方顺序或用户选的「最近更新」（`tree.ts:520-525`）。另外旧置顶项之间按置顶的先后排，现在按官方顺序排。→ **建议用户拍板**（旧行为更利于「先看有事要处理的」）。
13. **空工作区也能展开**：旧的空组 hover 不换成折叠三角、点击没反应（`sessionsWebview.ts:2366-2369`），现在照样能展开收起（展开后仍是空的）。→ **建议改**（小改动，回归旧行为）。
14. **工作区分组不能再拖拽排序**：旧的管理视图里可以拖动组行改顺序（`sessionsWebview.ts:1107-1168`），装配树没有这个入口（`pure/workspaceGroups.ts:97` 的 `reorderGroups` 没人调用）。→ **建议改**。
...
16. **内置标签组（待办 / 进行中 / 已完成）没有了**：旧侧栏每个工作区都预置这三组（`pure/sessionTags.ts:13-20`、`sessionsStore.ts:2146`），现在迁入时丢弃（`pure/sessionTagGroups.ts:24-28`、`52`）。→ **保持现状**（#98/#107 明确定了不恢复这套；要恢复请另立条目）。
17. **拖动会话时源行不再半透明**：旧的有 `.dragging { opacity: .45 }`（`sessionsView.ts:634`），现在拖拽过程中源行没有任何变化（目标组的高亮还在）。→ **建议改**（小改动）。
...
- 拖会话入组 / 拖出、拖 pill 换组序（连自定义 MIME 名字 `text/dsh-session` / `text/dsh-tag` 都一样）。
...
- 顶栏多了：视图选项（按工作区 / 单列表、手动序 / 最近更新）、设置齿轮、多选入口；搜索栏常显且是官方展开态。

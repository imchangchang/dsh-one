...
| 移到分组…（就地展开的二级项：各组 / 不归入 / 新建） | `sessionsWebview.ts:3337-3341`、`3406-3456` | `rows.ts:1054-1060`、`tree.ts:1041-1064` | 不同（子项内容：旧含三个内置组 + 「不归入分组」+「新建分组…」；现无内置组，含自建组 + 「不归入标签组」+「新建标签组…」。出现条件：旧**恒出现**，现只有这一行有标签组入口时出现——不在任何组块里的行不出现） |

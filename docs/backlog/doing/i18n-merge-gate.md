...
## 方案（2026-09-02 定稿：实现后置，仅记录）

在 `dev-merge.sh` 校验阶段加一步 i18n 检查（独立脚本 `scripts/check-i18n.sh`，可单跑；**后续实现，本次只记录**）：
...
- **文档层（2026-09-02 定稿）**：只查**对外 README**（`README.md` / `README.zh-CN.md`），**不含 `docs/`**。文档里引用用户可见文案的地方（命令标题、状态栏文本、界面标签）必须与 manifest nls / 状态栏文案一致——中文命令名出现在英文 README、或改了命令标题却忘了 README 都属于漏同步。检查方式：diff 涉及命令标题 / 状态栏文案改动时，扫描两个 README 里对应的旧字符串是否仍在；另加一次全量对照（英文 README 中不应出现中文命令引用，中文版本相反）。
...
- README.md（英文版）曾引用中文命令名（`DSH One: 打开 dsh 页面` 等 8 处），与 `package.nls.json` 英文标题不一致；已修正为英文命令名（提交 fc4378d）。README.zh-CN.md 中文命令名与 `package.nls.zh-cn.json` 一致，无需改。

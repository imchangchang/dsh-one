...
- 2026-09-03 @ 附件候选收窄：只列当前 composer 已附加的附件，历史消息截图不再混入
  （用户拍板：只出现附件内的照片文件，不出现所有截图）→ 仍 done

- 2026-09-03 用户拍板：截图命名 imgN（按会话目录递增）＋按会话分目录（<tmp>/dsh-one-attachments/<sessionId>/）＋
  缩略图底部名称横幅＋@ 弹窗分三组（附件/工作区/会话）＋@ 行间标点后触发 → 仍 done

- 2026-09-03 用户验收补充：hover 输入框 @token 联动对应附件 chip 高亮（文本高亮层 + rect 命中）、
  名称横幅 9px 两行完整显示 → 仍 done

- 2026-09-03 修复 hover 高亮：canonical/@前缀路径归一化后匹配、hover 独立样式（用户反馈高亮不正确）→ 仍 done

- 2026-09-03 收敛高亮语义：去掉点击选中的常驻高亮，只保留 hover 即时联动（用户：不悬停仍有一个图片高亮）→ 仍 done

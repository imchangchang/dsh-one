...
3. **插入形态（开发时决策点）**：我们的输入框是 textarea，做不到官方 contenteditable 的"可见标题、隐藏 mention"原子引用。两条路：
   - a) 直接插入 canonical mention 全文——简单可靠，但输入框里很长很丑；
   - b) 输入框显示 `@标题`，本地维护 标题→mention 映射，发送时替换——体验好，但要处理标题重复、发送后改名、手动编辑删改等边界。
   建议先做 a)，体验不可接受再升级 b)。

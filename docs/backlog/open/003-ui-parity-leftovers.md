...
3. ~~头部/hero 的 preset chip 无 tooltip 描述~~（已完成：roster description 带进头部 state，chip 悬停显示描述）。
...
8. ~~turn 中途被注入消息切断时操作栏重复出现~~（已完成：ChatAssistantMessage 加 `turnEnd` 标记，操作栏条件从 `m.complete` 改为 `m.turnEnd`）。

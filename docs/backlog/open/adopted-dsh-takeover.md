...
记录于 2026-09-02。现状：扩展探测到配置端口上已有 dsh（如用户在终端手动 `dsh web`）时复用（`adopted: true`），tooltip 明确提示「复用外部启动的实例，不会被插件停止」，且**隐藏「重启服务 / 停止服务」按钮**（statusbar.ts:46-52，`if (!status.adopted)`）。用户提议：外部启动的实例也应能接管，即用扩展的停止/重启也能关掉、重启它。
...
2026-09-02 讨论后用户拍板：**先不做**。保留现状（复用的实例不杀、隐藏停止/重启按钮）。原因：收益偏低（外部启动场景少见），成本是平台三套 pid 探测代码 + 确认弹窗 + 文档翻修改（architecture.md 决策 2「复用的实例永不 kill」、release-checklist 人工验收第 21 行）。以后要做时按上述调研直接动工。

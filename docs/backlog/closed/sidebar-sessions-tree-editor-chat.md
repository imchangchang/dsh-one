...
- 2026-09-01 验收打回（done → doing）：恢复全文搜索——官方 session.search 接口（dsh 0.1.1-rc.2）核实存在，改为该 RPC 实现内容命中 + snippet 展示 + 关键词高亮；后端未挂索引时降级为标题搜索。
...
- 2026-09-01 开发完成（doing → done）：运行中会话的菜单「标为已读/未读」置灰禁用（menuItem 支持 disabled）。
- 2026-09-01 主线合入测试通过（merge d3f3416：重放 59 提交零冲突 + typecheck/202 测试/build 通过 + 基线冒烟 14 场景核对），人工验收通过 → closed

...
- 2026-09-01 用户要求按「使用者/开发者」双视角重写 README：使用者（能做什么对齐当前功能全貌、快速开始、工作原理 mermaid、使用指南、配置项含 autoStart、安全、兼容性、卸载）+ 开发者（环境、命令、调试、架构总览、测试约定、发版、文档索引）；中英两文件同步，自测通过重打标 → done（commit aef1f43）
- 2026-09-01 主线 dev-merge 合入：rebase 到最新 main（6 个提交无冲突）→ 复测 typecheck/test(317)/build 全绿 → --no-ff 合入（merge 4073050）→ 清理 worktree/分支/done 标记并重建主线 dist → closed

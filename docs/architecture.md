...
2. **取清单**：带 cookie GET 网关 `/`，从注入 HTML 里取出 `__DSH_BOOT__` 清单与前端资产名（`extractBootWire` / `extractFrontendAssets`），版本不在 `[0.1.6-alpha.1, 0.2.0)` 时给页面顶部加一条信息条（**不阻断**，条上连安装命令一起给；区间的单一事实源是 `src/pure/versionGate.ts`）。
...
**版本门**：装配页期望 dsh `[0.1.6-alpha.1, 0.2.0)`。下界 = **整轮浏览器验证能全过的最老那一版**（#234 起；此前写的是 0.1.2-rc.1，实测那两代连「把某条会话变成当前会话」这条主路径都不成立）；上界之上没验过。这道门**不阻断**：区间外只在页面顶部出一条信息条，条上直接把安装命令给出来——今天没有任何 npm dist-tag 指向实测可用的版本（`latest` / `next` 都低于下界、`alpha` 那版实测不通），所以信息条写的是确切版本。区间取值、判定与那条命令的单一事实源是 `src/pure/versionGate.ts`（产品侧与装配实验室共用；此前两处各写一份常量）。逐版本读数与已知的不通项见 `docs/dsh-compat-checklist.md` 的「支持的 dsh 版本」，那一节同时记着这处已知不严：整区间判断表达不了「区间内但实测坏」的版本（`0.1.7-alpha.2` 就是这种）。
...
12. **页面上的插件 roster 有两条来路，两条都过滤。** 一条是启动时那份 `__DSH_BOOT__`，另一条是官方前端自己开的事件流 `/plugins/events`（每帧 `type: "graph"` 带全量 roster）。0.1.6-alpha.2 起客户端会**采纳**后者，不过滤就会把被 block 的官方插件装回来、把自有 frame 插件的条目卸掉，整页白。做法同源：页面把那条流改道到镜像的 `/plugins-local/events`，镜像逐帧跑**同一个** `filterWire`，投影不了就丢帧（退化成老版行为，比放行洗白页面安全）。**版本也要对齐**：0.1.6-alpha.2 起条目的 `rev` 是每进程随机值（`dsh-client-modules` 的 `randomBytes(8)` + `allocateInitialRevision()`），网关重启一次就全变，客户端按 rev 判「这一条变了没有」——整份名册的 rev 都变 = 每一条先拆后建，会话 scope 的对接件被撤销、官方渲染器抛 `SlotAssemblyError`、React root 卸载、整页白。所以页面改道时把自己 boot 那份清单的 `id → rev` 一并带上（`revs` 参数），镜像据此把推来的名册逐条对齐回这一页 boot 的值；id 集合真的变了（网关增删了插件）时原样下发。基线的格式是 JSON（`[[id, rev], …]` 再整体转义，见 `wireFilter.ts` 的 `ROSTER_REVS_PARAM`）：这份值里**真的会出现分隔符**——0.1.7 起自有条目的 rev 是「每个 application 批的 rev 用逗号并起来 + 本地内容哈希」，而 `id:rev` 逗号分隔那套格式假定分隔符不出现在值里，撞上就会被截断成错的基线、让客户端把那几条先拆后建（拆掉自有 frame 插件时 `root` 槽的注册随之撤销、整页白，#243）。同一版 dsh 里还有三处同类漂移（会话快照不再下发 `current`、会话服务删掉 `open` / `select` / `clear`、`completed` 挪进 `sessionStatus` 钩子），四处都在我们这边做了单一事实源 + 分代分叉，两代官方产物都能服务。
...
14. **版本门只提示不阻断。** 区间外的版本给一条信息条照常使用——挡掉用户会让他没法自查是谁的问题，而区间判定太粗（整段区间里已知有漂移），挡也不准。信息条上除了区间还给**装哪一版的那条命令**（区间取值与命令同源，见 `src/pure/versionGate.ts`）：今天没有任何 npm dist-tag 指向实测可用的版本，只说区间等于把用户晾在那里。
...
- `#230` 事件流推来的名册按**这一页 boot 那份清单**对齐条目 `rev`（0.1.6-alpha.2 的 `rev` 是每进程随机值，网关重启一次就全变，不对齐会被读成「整份名册都变了」→ 整页白）；实验室装配页的网关清单改为每次现取（同一件事的夹具侧成因：缓存一份会在网关重启后变成假话）。

...
- `src/ui/sidebarStatusPage.ts` / `src/ui/installGuide.ts`：侧栏状态页与安装引导 tab（#100）。两者都是**宿主侧渲染的普通 HTML**（我们自己的 HTML + CSS + 内联脚本，文案走 `vscode.l10n.t`），不参与装配树——dsh 未安装时网关起不来，装配页组装不了，这一层只能由宿主直接给页面。状态页三态的分流判定在 `src/pure/sidebarStatus.ts`（纯函数，单测覆盖）；引导 tab 是单例面板（槽位逻辑在 `src/pure/panelSlot.ts`），已开则聚焦。

...
| `0.1.6-alpha.2` | 实测通过 | 73 项全绿、3695 条断言零红（2026-09-23 实测，#238 这一轮的回归读数，含并入集成线之后的套件；此前记录的 F-61 / F-62 两条 chat 树 settle 红本轮都不再出现，见 #226） |
| `0.1.5-rc.3` | **实测通过** | 73 项全绿、3681 条断言零红（2026-09-23 实测，#238，含并入集成线之后的套件）。**它正是 npm `latest` 与 `next` 两个标签指的版本** |
| `0.1.5-rc.2` | **实测通过** | 73 项全绿、3681 条断言零红（2026-09-23 实测，#238，含并入集成线之后的套件）。**版本门的下界** |
...
`0.1.5-rc.2` **73 项全绿、3681 条断言零红**；`0.1.5-rc.3` **73 项全绿、3681 条断言零红**；同一天的回归读数 `0.1.6-alpha.2` **73 项全绿、3695 条断言零红**（此前记录的 F-61 / F-62 两条 chat 树 settle 红本轮都不再出现，见 #226）。三份报告与台账路径见 #238 的 comment。

...
- **#239 起还多一项 F-70**（树底「已归档」一节：取消归档的入口，判据与夹具见 `test/assembly-lab/README.md` 的 F-70 那一行）：它要侧栏树渲染出来才量得到，所以在这一版上同样被上面那条挡住——0.1.7 上它只记一条带签名的事实（`Minified React error #130`；签名不符当场判红），端到端由实验室夹具（就地声明 `sidebar.workspaces.session.menu.item` 那条槽位、把 0.1.7 那一代的形状摆出来）在 **0.1.6-alpha.2 上**承载（F-70 20/20）。#236 落地后这一档自动变成真 0.1.7 上的验证。**本节与上表各项读数都是 F-70 加入之前那一套（2026-09-23）；F-70 之后整轮是 71 项（70 个套件 + R-06，2026-09-23 本机在 0.1.6-alpha.2 上实测 3672 / 3672 条断言全过、零红）。**

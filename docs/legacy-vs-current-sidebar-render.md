...
> **写作时间与适用版本**：2026-09-17 起写、2026-09-18 随 #197 更新，对分支 `develop/cordis-chat`（该分支已在 2026-09-19 整体合入 `main`、随 2.0.0 发布）。**文中的截图与「当次实测」读数是那一刻的记录**，此后两侧都动过：B 表的几何读数只保证「写作时那份代码 + 那份 harness」下成立；文末几处建议后来陆续被采纳（#151 / #152 / #153 / #154 / #155，逐条见 C2 末尾那段）。**要看当前事实请读 `docs/legacy-vs-current-sidebar-compare.md`（源码级对照）与代码、`docs/architecture.md`**；旧侧栏（`src/ui/sessionsView.ts` 等）在 2.0.0 已摘钩，只作迁移参照物。

本文件是 issue **#129** 的产出（两阶段里的**第二阶段**：先源码级对照 #128，再实际渲染对比）。
它只出对照结论，**不改任何功能代码**。先做的那份源码级对照在
[`legacy-vs-current-sidebar-compare.md`](./legacy-vs-current-sidebar-compare.md)，本文末尾的
...
- **数据**：两侧吃的是**同一次**只读网关读取（`session/list` + `workspace/follow` 的基线帧）。
  ...
  **几何结论**（B 节那 65 项密度 / 规格读数）跟数据无关，复跑照旧。两侧「怎么读」本来就不同，
...
下面那几张是**并排图**：左旧、右现，同一宽度、1:1。完整的一套（9 个状态 × 260/340/500 三档，
...
（待交互 / 运行中 / 未读，收起的组上正是最有用的时候）；当次装配侧只有「运行中 / 等待交互」
两项（#128 的 C1-9）。**这一条后来改了**：第三项「未读」由 #153 补回，现在两边都是三项。
...
「‹ 返回 + 回收站 + 计数 + 清空 + 恢复全部」（47px 高），当次装配侧是「回收站 + 计数 + ✕」（26px 高），
清空与恢复全部只剩底部入口行那两枚（#128 的 C1-10）。块头高 32 → 24、行高 32 → 26。
**这一条后来改了**：抽屉头由 #154 补成「‹ 返回 / 清空 / 恢复全部」三样 + 标题与计数，行的右键菜单、
状态点与图钉也一并补回。
...
- 建议：**保持现状**（同一门密度口径）；抽屉头少的那两枚动作见 C1-10（**#154 已补回**）。
...
- 现象：旧侧栏组头行尾是三项（待交互 / 运行中 / 未读，各带图标与数字），当次装配侧是两项
  （运行中 / 等待交互）。折叠态里这个差别最刺眼——组收起时那行计数就是唯一的线索。
- 建议：**已改（#153）**：第三项「未读」补回来了，现在两边都是三项。

**C-8. 会话行内与行尾的内容取舍**（已在源码对照里列出：C1-12）
- 现象：同一棵工作区里，旧侧栏先把「运行中 / 有后代在跑 / 未读 / 待交互」的会话提到前面
  （活跃前置），再按最近更新排；现装配侧只有「置顶最前」，其余保持官方顺序（#131 起连「最近
  更新」那一档排序选项也退役了）。截图里同一棵工作区内的行序因此不同。
...
**这份对账之后陆续被采纳的条目**：C1-1 与 C1-13（#151 定了口径：跟官方一致，不改回旧行为）、
C1-3（#152 补回搜索高亮）、C1-9（#153 补回未读计数）、C1-10（#154 补齐抽屉头与行）、C1-14 与
C1-17（#155 补上分组拖拽排序与拖动源行半透明）。逐条处置见 `legacy-vs-current-sidebar-compare.md` 的 C1。
...
- **没起 VS Code 窗口**：这一轮只做浏览器验证（Playwright chromium），没做 VS Code 验证。这一层验不到的宿主行为
  （CSP 实际执行、原生菜单、主题跟随）由 `scripts/dev-ui-test.sh` 那一档负责。
- **数据是当天的**：会话内容随网关变化，几何读数（本文的 65 项）不随数据变——三档宽度、多次

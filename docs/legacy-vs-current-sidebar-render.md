...
| 子项文字左缘 − 父项文字左缘（#143 新增） | 14 | 0 | 见下方说明 |
| 子项文字左缘 − 父项图标槽右缘（#143 新增） | 22 | 6 | 见下方说明 |

说明：第一项的基准是**整个菜单第一项的图标列**，后两项（#143 加的）的基准是**父项自己**——
父子关系的三条关系量要一起看才说明白。
...
- 旧侧栏（正本）：父项 `.menu-item{padding:4px 10px;gap:8px}`，子项
  ...
  父项文字左缘 = 10 + 14 + 8 = 32（子项比父项深 **14px**），父项图标槽右缘 = 10 + 14 = 24
  （子项文字在它右边 **22px** = 子项自己的图标槽 14 + 行内间隙 8）——就是上表后两行。
- 现装配侧（#126 立、#143 重定）：菜单取官方紧凑档（项内边距 7 / 图标槽 14 / 间隙 6）。#126
  曾给子项整行加一个缩进位（左内边距 27px），子项文字比父项文字深 20px——用户实测报「太深」；
  #143 把子项的左内边距收到与父项同档的 7px：子项文字左缘 = 7 + 14 + 6 = 27 = 父项文字左缘
  （父项同一条算式），于是**深 0px**，同时落在「父项图标槽右缘（7 + 14 = 21）+ 一项内间隙」
  上（读数 +6px）。首行的 20 是「子项文字到整个菜单第一项图标列」的距离（= 第一项的图标槽
  14 + 项内间隙 6）：#129 当时（我们还没给子项加缩进）是 20，#126 期间是 40，#143 之后回到 20
  ——这一行的读数变过三次，看它时要连着版本读。

两边都是就地展开，形态一致（#128 的 A3.1）；层级在现装配侧由父项右端的 ▸/▾ 与子项自己的
图标槽承担，不靠缩进。这三条关系量在装配页上由 `test/assembly-lab/` 的 F-32 钉住。

## B9. 一致与不同的总数

65 项里 **18 项读数相同**、**47 项不同**、**0 项有一侧量不到**。相同的那些集中在：字号
（会话标题 / 工作区名 / 菜单项 / 分组胶囊 / 空态主文字）、置顶图钉、菜单项图标位与项间隙、
分组过滤胶囊的圆角、标签组那一整组（pill / 组头 / 竖线左缘 / 组内缩进）、回收站入口动作按钮、
空态左内边距。**三档宽度下没有一个几何项会变**——说明这 47 项差的是密度与规格，不是宽度挤出
来的。

（项数与一致/不同的数字是 #143 复跑时刷新的：这一轮把菜单二级那两条与父子关系直接相关的
量加进了表——B8 里 #143 新增的两行，`test/legacy-sidebar/` 的指标表同步。）
...
**C-11. 标签组二级菜单的缩进**（渲染才发现的；#126 / #143 已收口）
...
- 收口：#126 按「看得出层级」给子项整行加了一个缩进位（相对父项文字深 20px），用户实测报
  「太深」；**#143 把它收到与父项文字同列**（深 0px，读数见 B8），层级改由父项右端的 ▸/▾
  与子项自己的图标槽承担。装配页上的三条关系量由 `test/assembly-lab/` 的 F-32 钉住。

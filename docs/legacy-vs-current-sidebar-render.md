...
文字深 14px）；现装配侧自 #126 起整行缩进一档、#143 起收到**与父项文字同列**（相对父项文字深
0px，逐项读数见 B8 与 C-11）。
...
| 项 | 旧实测 | 现实测 | 差 |
| --- | --- | --- | --- |
| 二级项相对一级项**图标列**的缩进 | 16 | 34 | 见下方说明（两侧取的不是同一个东西） |
| 子项文字左缘 − 父项文字左缘 | 14 | 14 | **一致**（#126 是 20、#143 是 0、#167 起 14） |
| 子项文字左缘 − 父项图标槽右缘 | 22 | 20 | -2px（差的是项内间隙：旧 8、现 6） |
...
- 旧侧栏（正本）：父项 `.menu-item{padding:4px 10px;gap:8px}`，子项
  ...
  ——就是上表后两行。
- 现装配侧（#126 立、#143 收到 0、#167 取回一位）：菜单取官方紧凑档（项内边距 7 / 图标槽 14 /
  间隙 6）。#126 曾给子项整行加一个缩进位（左内边距 27px），子项文字比父项文字深 20px——用户
  实测报「太深」；#143 把子项的左内边距收到与父项同档的 7px，与父项文字同列（深 0px）——用户
  看下来又觉得**层级没了**；#167 用户拍板「留一点」，取值**从档位表里取**：左内边距 = 紧凑档的
  项内边距 7 + 紧凑档的图标槽 14 = 21px，于是子项文字左缘 = 21 + 14 + 6 = 41、父项文字左缘 =
  7 + 14 + 6 = 27，差 **14px = 一个图标槽**，与旧侧栏同值；子项文字相对父项图标槽右缘（7 + 14
  = 21）差 20px（= 子项自己的图标槽 14 + 项内间隙 6）。首行那 34 是「子项文字到整个菜单第一项
  图标列」的距离（= 21 + 14 + 6 − 7）：#129 当时（我们还没给子项加缩进）是 20，#126 期间是 40，
  #143 之后是 20，#167 起 34——这一行的读数变过四次，看它时要连着版本读；旧侧栏那一侧的 16
  里另含 2px（它量的是子项图标槽里那个 10px 色块，色块在 14px 槽里居中）。

两边都是就地展开，形态一致（#128 的 A3.1）；层级在现装配侧由父项右端的 ▸/▾ 与子项自己的
图标槽承担，缩进只留「一位」做辅助。这三条关系量在装配页上由 `test/assembly-lab/` 的 F-32 钉住。

## B9. 一致与不同的总数

65 项里 **19 项读数相同**、**46 项不同**、**0 项有一侧量不到**。相同的那些集中在：字号
（会话标题 / 工作区名 / 菜单项 / 分组胶囊 / 空态主文字）、置顶图钉、菜单项图标位与项间隙、
**菜单二级的「子项文字比父项文字深多少」（#167 起两边同为一个图标槽）**、分组过滤胶囊的圆角、
标签组那一整组（pill / 组头 / 竖线左缘 / 组内缩进）、回收站入口动作按钮、空态左内边距。
**三档宽度下没有一个几何项会变**——说明这 46 项差的是密度与规格，不是宽度挤出来的。

（项数与一致/不同的数字是 #167 复跑时刷新的：这一轮「子项文字比父项文字深多少」从 0 回到
旧侧栏那一档（14px），两条读数因此由「不同」变成「一致」——B8 里 #143 新增的那一行，
`test/legacy-sidebar/` 的指标表同步。）
...
**C-11. 标签组二级菜单的缩进**（渲染才发现的；#126 / #143 / #167 已收口）
...
- 收口：#126 按「看得出层级」给子项整行加了一个缩进位（相对父项文字深 20px），用户实测报
  ...
  子项文字比父项文字深一个图标槽（14px，读数见 B8，与旧侧栏同值）；层级仍由父项右端的 ▸/▾
  与子项自己的图标槽承担，缩进只作辅助。装配页上的三条关系量由 `test/assembly-lab/` 的 F-32 钉住。
...
回收站入口行字号 12 → 14、二级菜单缩进（旧 16 / 现 20，见 B8）。

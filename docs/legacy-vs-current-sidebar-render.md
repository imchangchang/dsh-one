...
「新建」（#128 的 A3.1 记的形态对得上）。差别是**缩进**：旧侧栏把子项整体右移 16px（相对父项
文字深 14px）；现装配侧自 #126 起整行缩进一档、#143 起收到与父项文字同列，**#167 取回旧侧栏
那一档、#171 再收一半**（相对父项文字深 7px，逐项读数见 B8 与 C-11）。
...
| 项 | 旧实测 | 现实测 | 差 |
| --- | --- | --- | --- |
| 二级项相对一级项**图标列**的缩进 | 16 | 27 | 见下方说明（两侧取的不是同一个东西） |
| 子项文字左缘 − 父项文字左缘 | 14 | 7 | -7px（我们的量 = 旧侧栏那一档的一半；#126 是 20、#143 是 0、#167 是 14、#171 起 7） |
| 子项文字左缘 − 父项图标槽右缘 | 22 | 13 | -9px（差的是项内间隙与那半格缩进：旧 8、现 6） |
...
- 旧侧栏（正本）：父项 `.menu-item{padding:4px 10px;gap:8px}`，子项
  ...
  ——就是上表后两行。
- 现装配侧（#126 立、#143 收到 0、#167 取回一位、#171 取一半）：菜单取官方紧凑档（项内边距 7 /
  图标槽 14 / 间隙 6）。#126 曾给子项整行加一个缩进位（左内边距 27px），子项文字比父项文字深
  20px——用户实测报「太深」；#143 把子项的左内边距收到与父项同档的 7px，与父项文字同列（深
  0px）——用户看下来又觉得**层级没了**；#167 用户拍板「留一点」，取值**从档位表里取**：左内边距
  = 紧凑档的项内边距 7 + 紧凑档的图标槽 14 = 21px，子项文字比父项文字深 14px，与旧侧栏同值
  ——用户看过仍觉得偏大；#171 取那一档的一半：左内边距 = 紧凑档的项内边距 7 × 2 = 14px，于是
  子项文字左缘 = 14 + 14 + 6 = 34、父项文字左缘 = 7 + 14 + 6 = 27，差 **7px = 半个图标槽**
  （= 紧凑档的一个项内边距）；子项文字相对父项图标槽右缘（7 + 14 = 21）差 13px（= 子项自己的
  图标槽 14 + 项内间隙 6 − 那半格缩进 7）。首行那 27 是「子项文字到整个菜单第一项图标列」的
  距离（= 14 + 14 + 6 − 7）：#129 当时（我们还没给子项加缩进）是 20，#126 期间是 40，#143 之后
  是 20，#167 起 34，#171 起 27——这一行的读数变过五次，看它时要连着版本读；旧侧栏那一侧的
  16 里另含 2px（它量的是子项图标槽里那个 10px 色块，色块在 14px 槽里居中）。

两边都是就地展开，形态一致（#128 的 A3.1）；层级在现装配侧由父项右端的 ▸/▾ 与子项自己的
图标槽承担，缩进只留**半格**做辅助（#171 起）。这三条关系量在装配页上由 `test/assembly-lab/`
的 F-32 钉住。
...
（项数与一致/不同的数字是 #171 复跑时刷新的：这一轮「子项文字比父项文字深多少」从 14（= 旧侧栏
那一档）收到它的一半 7px，读数与旧侧栏不再相等，这一条因此由「一致」回到「不同」——#167 复跑时
它刚从「不同」变成「一致」；B8 里 #143 新增的那一行，`test/legacy-sidebar/` 的指标表同步。）
...
**C-11. 标签组二级菜单的缩进**（渲染才发现的；#126 / #143 / #167 / #171 已收口）
...
- 收口：#126 按「看得出层级」给子项整行加了一个缩进位（相对父项文字深 20px），用户实测报
  ...
  的一半**——左内边距 = 紧凑档的项内边距 × 2（7 + 7 = 14px，档位表里那一个量用两遍），子项文字
  比父项文字深 7px（半个图标槽 = 一个项内边距，读数见 B8）；层级仍由父项右端的 ▸/▾ 与子项自己
  的图标槽承担，缩进只作辅助。装配页上的三条关系量由 `test/assembly-lab/` 的 F-32 钉住。

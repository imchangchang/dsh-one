 ...
 *    `lib/client.js`）：26 枚里有 18 枚被官方插件自己用到，**18 枚全取 `Regular`**，唯一
 *    一处 `Medium` 是输入框那枚 14px 加号（`IconPlusOutlineMedium, { size: 14 }`）与侧栏宽
 *    形态的「新会话」（`IconNewChatOutlineMedium, { size: 14 }`）。我们这 26 枚在插件里
 ...
 * 两套观感。代价说明白：老一代的图形按「实心描边」量出来的相对线宽中位数是 0.082（区间
 * 0.060~0.178），`Regular` 是 0.0625、`Medium` 是 0.0813——**取 `Regular` 比 0.1.6 上的观感
 * 细约四分之一**，那是官方 0.1.7 自己的口径变化，不是我们选错了。要偏回旧观感就把
 * `DEFAULT_ICON_WEIGHT` 改成 `'Medium'`（一行），或给表里某一枚单独写 `weight`。
 *
 * ### 线宽怎么量的
 *

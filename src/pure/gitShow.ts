/**
 * commit 卡查询的 git CLI 数据面（纯函数，便于单测）。数据来源与解析口径照搬
 * main 分支的 `src/pure/commitGit.ts`（旧自研聊天区的同一功能），这里只保留
 * 装配线（#65 批 1）用得到的那几个函数：
 *
 * - 用 git 二进制直接查提交（`git log --no-walk`），不依赖 vscode.git 扩展——
 *   窗口没打开该仓库时 commit 卡仍能显示（旧实现同款取舍）。
 * - 字段集与 vscode.git 的 getCommit 对齐（hash/message/author/commitDate/shortStat），
 *   GitHub 链接由 remote origin 推导（旧实现的 githubUrlFromRemoteUrl 口径）。
 *
 * 页面侧（git 卡片插件）与宿主侧（hostBridge 的 git.show 处理）共用本模块，
 * 保证两侧对「一条提交长什么样」只有一份定义。
 */
...
/** git log -s --format=<GIT_INFO_FORMAT> --shortstat 的一条提交记录。 */
 ...
 * 命令行短 hash 展示都用它。只留日期会导致页面 new Date() 解析丢时区偏移，
...
/**
 * 把 CLI 查询记录投影成页面用的提交信息（subject 首行 + 完整 message + 作者 +
 * 日期 + 变更统计 + GitHub 链接），与旧自研聊天区的 commitInfoFrom 同构。
 */
export function commitInfoFromShowRecord(
  sha: string,
  rec: GitShowRecord,
  githubUrl?: string,
): CommitInfoResult {
...
/** 查不到的提交（宿主 hostBridge 的 not-found 回执，页面据 found=false 显示「未找到」）。 */
export function commitNotFound(sha: string): CommitInfoResult {

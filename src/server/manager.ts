...
export class ServerManager implements vscode.Disposable {
  ...
  private async start(): Promise<ServerStatus> {
    ...
    if (port > 0) {
      ...
      if (probe === 'foreign') {
        ...
        if (free === null) {
          throw new Error(vscode.l10n.t('端口 {0} 被其他程序占用，且 {1}–{2} 均不可用', port, port + 1, port + PORT_FALLBACK_ATTEMPTS))
        }
        this.logger.warn(`port ${port} is occupied by a foreign service; falling back to ${free}`)
        void vscode.window.showWarningMessage(
          vscode.l10n.t('DSH One: 端口 {0} 被其他程序占用，本次已改用端口 {1}（未修改设置）', port, free),
        )
        spawnPort = free
      }
    }
  ...
  private spawnViaLauncher(
    ...
    return new Promise<number>((resolve, reject) => {
      ...
      const hangTimer = setTimeout(() => fail(new Error(vscode.l10n.t('dsh 启动器无响应（{0}s 超时）', 10))), 10_000)
      proc.stdout?.on('data', (d: Buffer) => (out += d.toString()))
      proc.stderr?.on('data', (d: Buffer) => (errOut += d.toString()))
      proc.once('error', (err) => fail(new Error(vscode.l10n.t('无法启动 dsh 启动器: {0}', err.message))))
      proc.once('exit', (code) => {
        if (settled) return
        settled = true
        clearTimeout(hangTimer)
        ...
        if (code === 0 && Number.isInteger(pid) && pid > 0) {
          ...
          reject(new Error(vscode.l10n.t('dsh 启动器失败 (code={0}): {1}', String(code), errOut.trim() || out.trim())))
        }
      })
    })
  }
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        void this.readLogTail().then((tail) =>
          fail(new Error(vscode.l10n.t('dsh 启动超时（{0}s）\n{1}', START_TIMEOUT_MS / 1000, tail))),
        )
      }, START_TIMEOUT_MS)
      ...
      const poll = setInterval(() => {
        ...
        void (async () => {
          if (!pidAlive(pid)) {
            ...
            fail(new Error(vscode.l10n.t('dsh 提前退出\n{0}', tail)))
            return
          }
          let candidate = port
          if (candidate === 0) {

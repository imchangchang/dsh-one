...
const SESSIONS_STYLE = `
  ...
  /* 回收站抽屉：从面板底部滑出，默认占据约一半高度，叠加在主列表上（主列表
     ...
     下拉到底松手 = 收起，点击提手（不拖动）也 = 收起。无遮罩直接叠——点击抽屉外区域即收起。 */
  .recycle-drawer {
  ...
  /* 提手条：全宽可拖区 + 顶部居中小横条（上拉扩大 / 下拉收起 / 点击收起）。 */
  ...
  /* 回收站入口行（面板底部固定，不随列表滚动）：外层 div（button 不能嵌套）——
     主区按钮（描边垃圾桶 + 标签 + 计数）点击打开抽屉；右侧常驻两个快捷图标按钮
     （清空 / 恢复全部，复用 .sessions-tool）。计数为 0 时灰态。 */
  .recycle-entry {
  ...
  /* 主区按钮：压掉全局 button 的实底样式（原 .recycle-entry button 的重置挪到这里）。 */
  .recycle-entry-main {
    flex: 1; min-width: 0; display: flex; align-items: center; gap: 6px;
    padding: 7px 4px 7px 14px; margin: 0; border: 0; border-radius: 0;
    background: transparent; color: inherit;
    font: inherit; text-align: left; cursor: pointer;
  }
  .recycle-entry-main:hover { background: var(--vscode-list-hoverBackground, rgba(127,127,127,.12)); }
  .recycle-entry .sessions-tool { width: 26px; height: 26px; }
  ...
  /* 不变量：行尾内容缘 = 面板右缘 −10px（margin-right 4px + padding-right 6px），
     顶层行与组内行共用。行的水平盒模型完全归行自己管，任何包行的容器
     （.sessions-list / .workspace-row / .tag-group 等）不得加水平 margin/padding，
     否则行尾状态标记会推离这一列（#8 的教训）。组内行的左侧缩进也由行自己
     的 .session-row.tagged padding-left 承担，不靠容器。 */
  .session-row {
  ...
  /* 置顶图钉：标题前常驻（行首状态槽已移除，整行缩进更紧凑）。main 的
     flex gap 为 8px，-2px margin 收回到与行 gap 一致的 6px。 */
  .session-pin {
  ...
  /* 行尾状态槽：与时间互斥（有标记就不显示时间）。槽固定 16px、标记居中——
     绿点（6px）/黄点（6px）/像素环（10px）宽度不同仍中心共线；align-self
     覆盖 .session-main 的 baseline，垂直与行中心对齐。空闲时该槽不渲染，
     时间文本右缘贴行尾（.session-time）。 */
  .session-rear {
  ...
  /* 组块只做垂直节奏和装饰（pill/竖线），水平 margin 必须为 0——加水平 inset 会把
     组内行的行尾内容缘推离面板 −10px 列，与顶层行错开（见 .session-row 的不变量）。 */
  .tag-group { position: relative; margin: 4px 0 2px; }
  /* 组头：非常小的标签 pill，左缘与竖线同列（缩进列）。padding-right 把折叠组头的
     计数角标右缘对齐到状态列（面板 −10px，与 workspace 组头 badge 同列）。 */
  .tag-head { display: flex; align-items: center; height: 22px; padding-left: 16px; padding-right: 10px; }
  .tag-pill {
  ...
  .tag-line {
    position: absolute; left: 16px; top: 19px; bottom: 2px; width: 2px;

...
export const BLOCK_LIST: ReadonlyArray<{ id: string; reason: string }> = [
  {
    ...
    reason: '官方应用外框，与 VS Code 外壳冲突；由 @dsh-one/vscode-shell 接管根组合并提供 layout 服务',
  },
  {
    id: '@deepseek-ai/dsh-client-ui-sidebar',
    reason: '官方侧栏，VS Code 侧栏由自研面板承担',
  },
]
...
export function extractBootWire(html: string): BootWire {
  ...
  if (m === null) throw new Error('assembly wire: 网关 HTML 里找不到 __DSH_BOOT__ 注入')
  try {
    return JSON.parse(m[1]) as BootWire
  } catch (err) {
    throw new Error(`assembly wire: __DSH_BOOT__ JSON 解析失败: ${err instanceof Error ? err.message : String(err)}`)
  }
}
...
export function extractFrontendAssets(html: string): GatewayAssets {
  ...
  if (moduleJs === undefined) throw new Error('assembly wire: 网关 HTML 未找到 module script 资产')
  ...
  if (preloadJs.length === 0 || css.length === 0) {
    throw new Error('assembly wire: 网关 HTML 未找到 modulepreload/stylesheet 资产')
  }
...
export function filterWire(wire: BootWire): BootWire {
  ...
  if (dropped.length !== BLOCKED_IDS.length) {
    ...
    throw new Error(`assembly wire: 网关清单里没有预期的 blocklist 项: ${missing.join(', ')}`)
  }
  ...
  if (app === undefined || bootstrap === undefined) {
    throw new Error('assembly wire: 缺 bootstrap/application 批')
  }
  const keptIds = app.entries.filter((id) => !blocked.has(id))
  if (keptIds.length !== app.entries.length - BLOCKED_IDS.length) {
    throw new Error('assembly wire: application 批里的 blocklist 项与 entries 不一致')
  }
  ...
  return {
    ...
    batches: [
      ...
      {
        phase: 'application',
        // mirror 的 /plugins-local：按 kept ids 请求，mirror 拉官方原 combo
        // 剥 blocked 段后伺服；rev 沿用网关原值（缓存键）。
        url: `/plugins-local/??${keptIds.map((id) => `${id}/client.js`).join(',')}&rev=${app.rev}`,
        rev: app.rev,
        entries: [...keptIds, SHELL_PLUGIN_ID],
      },
    ],
  }
}

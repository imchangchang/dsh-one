 ...
 * 移出标签组 / 改名 / 颜色 / 删除组）。
 *
 * 为什么是「一条小标题 + 一节颜色」而不是二级子菜单：官方 `Menu` 的项只有
 * label / icon / disabled / danger / type 几个槽（`separator` / `label` 两种类型项在
 * 本插件的行菜单里已经在用），没有子菜单这一档；颜色那 6 项因此按工作区行菜单
 * 「所属分组」同一形态摊成一节（标题「颜色」+ 6 个色块项），当前色走官方 `selectedIds`
 * 打勾。菜单项只在菜单里出现，不做常驻占位。
 *

...
const STYLE = `
  ...
  #input {
    ...
    /* 原生按内容定高（Chromium 123+，VS Code ≥1.96 内嵌 Chromium 128）：
       取代 JS autoGrow 的 height:auto 塌缩测量——塌缩瞬态会 clamp 兄弟滚动
       容器的 scrollTop（回归 composer-input-jitter-pinned-scroll）。
       不支持的旧环境走 webview.ts autoGrow 的塌缩+修复兜底。 */
    field-sizing: content;
  }
  /* autoGrow 回退路径的隐藏测量镜像（无 field-sizing 的旧环境用，JS 在
     composer-frame 里动态创建）：与 #input 同宽同盒模型同字体，脱离流
     （position:absolute）不碍布局，value 同步后读 scrollHeight 得到目标高度——
     全程不对存活 textarea 做塌缩。 */
  .grow-mirror {
    position: absolute; top: 0; left: 0; right: 0; height: auto;
    visibility: hidden; pointer-events: none;
    box-sizing: border-box; padding: 6px 8px; border: 1px solid transparent;
  ...
  /* 输入框文本高亮层：与 #input 同字体流叠加，@ 引用 token 有底色；
     鼠标悬停 token 时 .active 加深并联动附件 chip 高亮。盒子用
     inset+overflow:hidden 锚死在 textarea 上；textarea 内部滚动时只平移
     内部 .ref-token-scroll（内容），盒子本身不动——否则长文本 scrollTop
     会把整个层顶出输入框（回归 composer-long-text-overflow）。 */
  ...
  /* 滚动同步的内容层：文本/token 挂这里，translateY(-scrollTop) 只作用于此层，
     由 .ref-token-layer 的 overflow:hidden 裁出与 textarea 一致的可见窗口。
     块级 div 默认撑满内容盒，无需额外布局规则。 */

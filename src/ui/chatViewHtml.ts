/**
 * Chat editor panel 的 HTML 与样式（`dshOne.chatPanel` webview 壳）。从
 * chatView.ts 拆出：面板 HTML 与宿主逻辑分离，ChatTabHost 创建 panel 时
 * 引用这里的 chatHtml。样式常量体量最大（对齐 dsh web 的全部 CSS），
 * 独立成文件让 chatView/chatTab 的宿主逻辑文件保持可读。
 */
...
const STYLE = `
  ...
  /* 聊天内容区（消息流 + 用户气泡 + markdown/表格 + 流内说明）正文字号随
     ...
     字号挂在 flow-col 而非 .messages：composer-seat 也在滚动容器内，不能让它
     继承内容字号。 */
  .flow-col { font-size: var(--dsh-content-font-size); }
  ...
  /* 断连横幅（重连状态透出）：chatCol 顶部整宽横条，消息流/内容列之外——不套
     .flow-col 的 --dsh-content-width 限宽（与头部 .chat-header 同为全宽 + 内边
     距），布局是 chatCol 新增一个 flex 首子项，不挤占消息流容器。三态：
     connecting 琥珀 + 转圈（自动重试中）、failed 红（阈值后仍后台重试，
     按钮=立即重连）、recovered 绿（短暂显示后 JS 自动隐藏，按钮隐藏）。 */
  .reconnect-banner {
    flex: none; display: flex; align-items: center; gap: 8px;
    padding: 6px 12px; font-size: 12px; line-height: 18px;
    border-bottom: 1px solid var(--vscode-panel-border, rgba(127,127,127,.3));
    background: var(--vscode-editorWarning-background, rgba(204,167,0,.12));
  ...
  .reconnect-banner.recovered {
    background: rgba(115,201,145,.14);
    color: var(--vscode-testing-iconPassed, #73c991);
  }
  .reconnect-banner-icon { flex: none; display: inline-flex; align-items: center; }
  /* 横幅内转圈沿用 .spinner 形状，颜色随三态 currentColor（边框/扫光继承）。 */
  ...
  .reconnect-banner-btn {
    flex: none; padding: 1px 10px; font-size: 12px; line-height: 18px;
  ...
  .reconnect-banner.recovered .reconnect-banner-btn { display: none; }
  .messages {
    flex: 1; overflow-y: auto; padding: 8px 12px 0;
    display: flex; flex-direction: column; gap: 10px;
    /* 双层防线（条目 scroll-bottom-momentum-jitter，见 docs/backlog/closed-archive-2026Q3.md）：
    ...
    /* 增量更新（chat-render-incremental-update）：消息流从全量重建改为按行对账后，
       顶部补页（loadEarlier）/ 行插入会命中 Chromium 原生 scroll anchoring，叠加
       render 尾部的程序补偿（prevScrollTop + ΔscrollHeight）会双重补偿。关掉原生
  ...
  /* 居中内容列（对齐 dsh web EvIC1a_column）：所有流内元素是这一个列容器的
     ...
     flex:1 0 auto：内容短于视口时把 composer-seat 推到底部（seat 是滚动容器
     内末位项，不做绝对定位浮层）。 */
  .flow-col {
    flex: 1 0 auto;
  ...
  /* composer 坐席（对齐官方 dsh web 的 data-composer-seat）：滚动容器内部
     末位项、sticky bottom——composer/dock 家族（todo/goal/queue/pending/
     input-area）全在这里面。它们增高只把滚动内容往上顶，**不再压缩消息区
     clientHeight**，V 类扰动（塌缩 clamp→补偿抖动）在布局层消失（回归
     composer-input-jitter-pinned-scroll / composer-multiline-input-jitter）。
     margin 负值抵消 .messages 的水平 padding，seat 保持通栏（border-top 横贯）。
     背景必须不透明：上滚时内容从 seat 下面滑过。 */
  .composer-seat {
  ...
  /* 等待插话的 steering 消息：与正常用户消息同样的气泡，左侧加处理中圆圈。
     .steering-line 撑满行宽，气泡 max-width:85% 按整行解析——与插话落地后的
     正式用户消息一致；spinner 只对气泡行垂直居中（gap 6px 紧贴气泡）。 */
  .msg.user .steering-line {
  ...
  /* commit 悬浮详情卡（仿 VS Code 内置 Git 的 commit hover 卡）：
     深色浮层、作者行（图标+名字+时间）、message 全文、分隔线、变更统计、
     命令行（短 hash 点开 commit / 复制 / Open on GitHub）。 */
  .commit-card {
  ...
  .commit-card-meta { font-size: 12px; color: var(--vscode-descriptionForeground, #888); }
  .commit-card-author svg { color: var(--vscode-descriptionForeground, #888); flex: none; }
  .commit-card-author-link { color: var(--vscode-textLink-foreground); text-decoration: none; }
  .commit-card-author-link:hover { text-decoration: underline; }
  .commit-card-time { display: inline-flex; align-items: center; gap: 3px;
    color: var(--vscode-descriptionForeground, #888); font-size: 12px; }
  .commit-card-msg { margin-top: 6px; font-size: 13px; line-height: 1.45;
    white-space: pre-wrap; word-break: break-word; }
  .commit-card-subject { font-weight: 600; }
  .commit-card-body { color: var(--vscode-descriptionForeground, #888); font-size: 12px; }
  .commit-card-sep { margin: 8px 0; border-top: 1px solid var(--vscode-widget-border, rgba(127,127,127,.3)); }
  .commit-card-stat { font-size: 12px; color: var(--vscode-foreground, #cccccc); }
  .commit-card-stat-add { color: var(--vscode-scmGraph-historyItemHoverAdditionsForeground, #73c991); }
  .commit-card-stat-del { color: var(--vscode-scmGraph-historyItemHoverDeletionsForeground, #f48771); }
  .commit-card-commands { display: flex; align-items: center; gap: 6px; margin-top: 6px; }
  .commit-card-cmd, .commit-card-copy {
    display: inline-flex; align-items: center; gap: 4px; padding: 2px 6px;
    background: transparent; border: none; cursor: pointer;
    color: var(--vscode-textLink-foreground, #4daafc); font-size: 12px; font-family: inherit;
  }
  .commit-card-cmd:hover, .commit-card-copy:hover { background: rgba(127,127,127,.15); border-radius: 4px; }
  .commit-card-copy svg { color: var(--vscode-foreground, #cccccc); }
  .commit-card-copy.commit-card-copied svg { color: var(--vscode-charts-green, #89d185); }
  .commit-card-commands .commit-card-cmd .commit-card-cmd { padding: 0; } /* 防嵌套加成 */
  ...
  /* 内嵌本地路径图片（工具输出 ![img](/abs/x.png)）：host 读盘回 data URL 后真图
     （.md-img-inline）；加载中占位（.md-img-loading）；宿主失败态 chip
     （.md-img-failed，点击在编辑器打开）——渲染逻辑见 webview decorateMarkdownImages。 */
  .md img.md-img-inline { display: block; margin: 8px 0; border-radius: 6px; }
  .md span.md-img-loading {
    display: inline-block; min-width: 160px; min-height: 90px; margin: 8px 0;
    border-radius: 6px; background: var(--vscode-editorWidget-background, rgba(127,127,127,.1));
    color: var(--vscode-descriptionForeground, rgba(127,127,127,.8));
    font-size: 0.85em; vertical-align: middle; text-align: center; line-height: 90px;
  }
  .md span.md-img-failed {
    display: inline-flex; align-items: center; gap: 6px; margin: 8px 0;
    padding: 3px 10px; border-radius: 6px;
    background: var(--vscode-editorWidget-background, rgba(127,127,127,.1));
  }
  .md span.md-img-failed svg { flex: none; }
  .md span.md-img-failed:hover { text-decoration: underline; }
  .md span.md-img-failed .md-img-failed-name { font-weight: 600; }
  .md span.md-img-failed .md-img-failed-hint { opacity: 0.7; font-size: 0.85em; }
  .md hr {
  ...
  /* 语法高亮（shiki，css-variables 主题）：token 颜色不写死在 HTML 里，走
     --shiki-token-* 变量。配色取自官方 dsh web 的 theme 包（浅色一套 + 深色
     一套），背景/前景沿用 VS Code 的代码块底色与正文色。未着色的文本落在
     --shiki-foreground。 */
  body {
    --shiki-foreground: var(--vscode-editor-foreground, #cccccc);
  ...
  body.vscode-dark, body.vscode-high-contrast, body[data-theme='dark'] {
  ...
  .md-code pre[data-hl] code { color: var(--shiki-foreground); }
  .reasoning {
    border-left: 2px solid var(--vscode-panel-border, rgba(127,127,127,.4));
  ...
  /* 展开块内容底部的「收起」小按钮（思考/工具卡/命令输出/压缩摘要等统一挂
     attachCollapseFooter）：观感对齐 .md-code-toggle，小灰字、hover 变亮。 */
  .details-collapse {
    display: block; background: none; border: none; cursor: pointer;
    opacity: 0.6; margin: 2px 0; padding: 0; font-size: 0.85em;
    font-family: inherit; color: inherit;
  }
  .details-collapse:hover { opacity: 1; }
  .tool-disclosure .details-collapse { margin-left: 20px; }
  .compaction .details-collapse { margin-left: 26px; }
  ...
  /* 窄面板隐藏轨道栏（回归 turn-rail-narrow-overlap）：最宽可见刻度 28px
     （frame right:6 + mark right:2 + mark 最宽 20px，活跃/hover 常态宽），
     文字右缘到 slot 右缘的间隙 =(contentBox-680)/2 = (W-704)/2 ≥ 28 需要
     面板 ≥760px；748px 以下间隙必不足、刻度压用户气泡右缘（实测 600px 重叠
     34px、740px 重叠 16px）。用 @container 而非 @media：显隐跟面板宽（嵌在
     窄编辑器组/侧栏里也算窄），与 diff 的窄容器退化同款。 */
  ...
  .goal-bar {
    display: flex; align-items: center; gap: 10px;
    box-sizing: border-box; min-height: 36px; padding: 4px 6px 4px 12px;
  }
  ...
  .command-row .command-line {
    font-family: var(--vscode-editor-font-family, monospace);
    /* #42：命令名 + 长 args 若超过容器宽（如 /goal 连续长串一行塞进 command-line
       span），默认 min-width:auto（flex 项）= 内容 min-content 宽（不可断），把
       .command-row 撑出横向溢出。min-width:0 允许收缩 + 任意断行让其在容器内换行。 */
  ...
  .command-detail summary .command-line {
    /* flex:none（0 0 auto）固定为内容宽，长 args 时同 .command-row 一样被 min-content
       撑宽横向溢出；0 1 auto 允许收缩，配合下方断行在 summary 内换行。 */
    flex: 0 1 auto; min-width: 0; overflow-wrap: anywhere; word-break: break-word;
  }
  ...
  /* 文件 chip 名称：小字号紧凑排版，最多两行完整显示（pasted-1.txt 这类
     文件名不再截断成 paste…；与图片横幅同风格）。 */
  .file-chip .chip-name {
    max-width: none; padding: 0 2px 1px;
  ...
  /* hover @ 引用 token 时的附件 chip 高亮描边（hovered，即时态；移出即恢复）。 */
  .attach-thumb.hovered, .file-chip.hovered {
  ...
  #input {
    ...
    /* 文字统一用输入前景色：@ 引用 token 由 Lexical 的 .ref-token 在真实文本流里
       高亮（不再是透明文字 + 叠加层），光标颜色同前景色。 */
    color: var(--vscode-input-foreground); caret-color: var(--vscode-input-foreground);
    background: var(--vscode-input-background);
    border: 1px solid var(--vscode-input-border, transparent); border-radius: 6px;
    font-family: inherit; font-size: inherit; line-height: inherit;
    /* 空内容时块级 contenteditable 高度塌陷到 padding+border，暂用 min-height 保
       住单行高度；有内容时随文本自然增高（max-height 钳上限，超长内部滚动）。 */
    min-height: 1.5em;
    max-height: 160px; overflow-y: auto;
    white-space: pre-wrap; overflow-wrap: break-word; outline: none; cursor: text;
  }
  #input::selection { background: var(--vscode-editor-selectionBackground, rgba(101, 158, 254, .35)); }
  #input:focus { outline: 1px solid var(--vscode-focusBorder); }
  /* 占位符层：绝对定位在 composer-frame 内（与编辑器的文字原点对齐：border 1px +
     padding 6px/8px），内容为空时由 composerEditor 控制显隐。 */
  ...
  /* 一键清空按钮（本地增强，官方 dsh web 无此按钮）：input-row 内、发送按钮
     左侧的 ghost ×，仅 composer 有内容（文本/待发附件任一）时显示——webview.ts
     按 hidden 属性切换。不设 display（hidden 需要 UA 的 display:none 生效）；
     flex 流内定位：既不与附件 chip 自带的 ×（chip 右上角 absolute）重叠，
     也不受 hero 大圆角卡片布局影响。 */
  .clear-all-button {
    flex: none; width: 20px; height: 20px; padding: 0; border: 0; border-radius: 6px;
  ...
  /* 「回到最新」浮标（对齐 dsh web EvIC1a_toBottomSlot）：sticky 零高槽位 +
     ...
     bottom 跟 composer 高度联动：--dsh-composer-height 由 ResizeObserver 观测
     .composer-seat 后写到本容器（对齐官方 --dsh-composer-height 链路），
     composer 增高时浮标自动上移，无 JS 重排。 */
  .jump-slot {
    position: sticky; bottom: calc(var(--dsh-composer-height, 0px) + 16px); height: 0; z-index: 5;
  ...
  /* 打开失败提示（open-error）：后端错误原因行——等宽小字、可选中复制、长文本换行。 */
  .open-error-reason {
    max-width: 520px; margin-top: 6px; padding: 6px 10px;
    font-size: 11px; line-height: 1.5; font-family: var(--vscode-editor-font-family, monospace);
    word-break: break-word; user-select: text;
    background: var(--vscode-inputValidation-errorBackground, rgba(186, 22, 22, 0.1));
    border: 1px solid var(--vscode-inputValidation-errorBorder, rgba(186, 22, 22, 0.4));
    border-radius: 4px;
  ...
  .install-script-copy {
    flex: none; width: 26px; height: 26px; padding: 0; margin: 0;
    display: inline-flex; align-items: center; justify-content: center;
    background: transparent; color: var(--vscode-descriptionForeground, #888);
    border-radius: 6px;
  }

...
const STYLE = `
  ...
  /* 行内码（反引号）交互态：轻背景承载 hover 的复制图标与路径点击（见
     ...
     图标默认隐藏（悬停/键盘聚焦才显示），pointer-events:none 防误触；hover 时
     浮现：16px 稍大命中区，right:-7px 让图标骑跨 chip 右缘（约 9px 叠在灰底上，
     与 chip 相连不悬空——用户实测反馈：悬空小图标鼠标移到按钮半路 hover 就
     断、点不到），图标自带同色系浅底与 chip 融合；absolute 相对 code 且部分
     在 code 内，鼠标从 chip 内滑到图标全程 hover 保持。路径码 hover 下划线
     提示可点，cursor 由 JS/role 语义给出。 */
  .md code.inline-code {
    position: relative;
  ...
  .md code.inline-code:hover .inline-code-copy,
  .md code.inline-code:focus-within .inline-code-copy {
    opacity: 1;
    pointer-events: auto;
  }

...
- 插件入口：从侧栏顶部那一条整行挪到侧栏工具栏（设置齿轮左边，悬停提示「插件」），原来那条整行不再占位；这一枚只在带官方插件页的 dsh（0.1.6-alpha.2 起）上出现，更老的 dsh 上侧栏里没有它，命令面板打开它也只给一行说明。

## [2.0.9]

### Fixed

- 插件页：dsh 0.1.6-alpha.2 起侧栏有「插件」这一条，点它现在打开的是官方插件页（按「官方 / 已安装」两组列出插件、逐行带启停开关），页里终端 / Agent 循环 / Subagent / 网页搜索四张配置卡也在；命令面板里另有一条「打开插件页」，可以直接开它。
- 设置页：第一次打开会看到官方的引导——先是「内测声明」，点「继续」之后如果没有任何能用的模型，接着引导填一次官方 API Key（也可以点「稍后配置」略过）；走完就不再出现。
...
- 归档之后在哪儿还原：0.1.6 上在设置页的「已归档会话」那一节；0.1.5 线上官方没有取消归档，归档之后不可逆；把入口搬进侧栏的那一版预览版上，由侧栏底部的「已归档」一节提供。

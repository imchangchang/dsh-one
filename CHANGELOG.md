...
- 局域网访问：开启后同一局域网内的设备能用带 token 的链接打开 dsh 网页；状态栏气泡标明当前是否局域网可达，关着时可一键重启切换（dsh 出于安全不允许直接监听 0.0.0.0，dsh One 以本机转发方式实现，dsh 本身仍只监听 127.0.0.1）。
...
- 侧栏菜单：会话行「移到分组…」与工作区行「分组…」展开出来的二级项比父项缩进得更少——从原来的一整格收成半格（半个图标位的宽度），看着仍是嵌在父项下面的一层，又不显得偏深；也不再像更早那样深两格。

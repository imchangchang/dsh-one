...
- 代码块语法高亮：回复里的代码按语言着色（内置 23 种常见语言，按需加载），只有滚进视口的代码块才着色。
- 输入框：文件可以直接拖进聊天面板成为附件，拖动经过时显示落点提示。
- 输入框：粘贴或拖入图片超过张数、单张大小上限时当场拦下并提示，不再无限制地收下。
- 输入框：输入 `/` 弹出命令补全菜单，命令名支持模糊匹配（`/pm` 能命中 `/permission`）。
- 输入框：选中 `/goal` 这类带参数的命令后进入参数模式，占位符直接显示该命令的参数说明。
- 输入框：`@` 引用的候选与 dsh 网页版同一份，路径按当前目录相对显示。
- 输入框：`@` 可以逐层下钻目录挑文件，弹窗顶部有路径面包屑，点任意一段即可退回该层。
- 输入框：运行中草稿为空时按 ⌘/Ctrl+Enter，一次把排队中的消息全部插话到当前回合。
- 回合过程折叠：跑了很多工具的回合收成一行「N 次工具调用 · M 条消息」，点一下展开看全过程，回复紧跟在折叠行下面，不用翻过一屏工具卡才看到答案。

### Fixed

- 中断的回合：你刚发出消息就取消时，聊天里不再多出一条空白行加「已中断」。
- 模型重试：一连串重试只把最后一次标成「已取消」，不再给每一行都挂上取消标记。
- 插话：插话落地后保持插话的样子，不再在落地那一刻整行闪一下重建。
- 输入法：候选窗打开时按 Esc 只关候选窗，不再连带打断正在跑的回复。
- 输入框撤销：清空输入后按撤销键能整体恢复刚清掉的文本与附件；回复发出去之后按撤销键不再把已发送的内容放回输入框。
- 输入框发送：带附件发送后输入框不再残留刚发出去的那段文字。
- @ 引用：从输入框复制出去的引用、以及跨窗口或重启后恢复的草稿，引用都是能直接用的完整路径，不再退化成解析不出的短名。
- 输入框引用：只剩一个 @ 引用时也能正常高亮（此前单条引用不着色）。
- 输入框清空：清空后按 Ctrl+Z（macOS 也可用 ⌘Z）能找回刚清掉的内容。
- @ 补全菜单：候选还在加载时菜单不再闪一下消失，改为显示「正在加载」；鼠标移到候选行即选中，目录候选按 Tab 直接进入下一层。
- 输入法：中文/日文候选确认那一下的 Enter 不再被当成发送。
- 剪贴板：同时复制了文件和文字时，文字不再被丢掉。
- 引用展开：手打的 @ 名称不再被此前引用过的同名单词悄悄替换成长路径。
- 附件与引用：鼠标悬停附件缩略图会点亮正文里对应的 @ 引用，反之亦然。
- 附件发送：附加的文件在 dsh 官方网页界面里显示为文件引用，不再是一行裸文本路径。
- 输入草稿：草稿里的图片按体积上限保存，不再让草稿文件无上限膨胀。
- 发送失败：回填的草稿不再覆盖你在等待期间新输入的内容。
- 主题：输入框引用高亮与图标颜色跟随浅色/高对比度主题。
- 消息顺序：会话历史重新加载或重连基线后，对话消息、排队列表与插话按发送顺序正确排列，不再错位。
- 输出区：流式输出时不再来回跳动，视图跟随更平滑。
- 发送反馈：回车后气泡与排队行立即出现，不再等一轮往返才看到。
- 断流重连：已经不存在的排队消息、后台任务与状态标记不再残留。
- 断流重连：会话列表与运行状态重新拉取，侧栏运行标记和停止按钮不再停在旧值。
- 切换会话：切走再切回，滚动位置与展开状态（思考、工具卡、代码块）保持原样，不再漂移或全部折叠。

### Changed

- 滚动：只有用户主动发送（发消息 / 插话 / 排队项转插话）才把视图滚到最新；用户已上翻读历史时，流式增长与插话 / 排队落地不再把视图拽回底部。
...
- 输入框 @ 引用分两段显示：还没选中的 @引用显示为可修改的高亮文本，从补全菜单选中后才变成带图标的只读小标签（悬停显示完整路径）。

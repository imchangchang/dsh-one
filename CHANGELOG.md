...
- 界面裁剪：去掉一批「本来就不会显示」的官方组件的拦截项（侧栏树那批用不到的对话区组件、两件不画任何东西的组件），界面看起来与以前一样；升级 dsh 后更不容易因为官方改名而受影响的可能更小。
- 界面隐藏的做法收敛：设置页里不显示的那两处官方设置项、侧栏的折叠钮与「新会话」按钮改用更可靠的判断方式隐藏，界面看起来与以前一样。
- 「会话正被另一个 dsh 占用」那条提示改用更稳定的依据来判断：在侧栏点开一条被另一个 dsh 打开的会话仍然给这条提示，文字与停留时间都不变。
- 自有插件包各自装完整：每个插件包目录里就是该插件的全部源码，为后续单独发布到 npm 做准备。
- 自有插件整理成可安装的插件包：同一份插件既供 VS Code 用，也能装进 dsh 网页版，为后续双端分发打底。
- 交互自检：新增一条自动检查，把关键控件逐个点一遍，点下去没有任何反应的会被点名报出来。本次据此查明「＋（添加文件或调用指令）」点不动的原因在配套的模型插件里（它注册指令时漏填了一个必填项，已在该插件仓库立 issue），dsh-one 这边没有可改的地方。

...
| 侧栏状态页（未安装 / 启动中 / 未运行 / 装配失败） | `src/ui/sidebarStatusPage.ts` + `src/pure/sidebarStatus.ts` | #100 交付，#101 纳入本覆盖 |
...
这两页恰恰是「dsh 还没装」或「服务没起来」时给用户看的——它们不参与装配树，网关起不来也要能显示，
所以既不需要网关，也不需要实验室那套（假宿主 + 真实模块构建 + 上游 wire）。这两页的问题
（下拉开不了、命令没跟着平台换、复制没反馈、状态页三态画错、明暗主题下看不见）用一个普通浏览器
页面就能全部测到。
...
| 启动失败 | 上面那两条 + 一段等宽详情块：错误文字原样显示，带 `<` `>` `&` 也当文本（不被当成标签、注入不生效） |

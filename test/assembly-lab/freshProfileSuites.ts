 ...
 *
 * 与 #162 / #177 的分工：`#162` 那条做的是**整轮套件跑在空实例上**（普查全部套件的
 * 判据自足性）；#177 起默认跑法就是「自起一台隔离实例 + 往里播种真数据」，本条仍只钉
 * **装载这一层**（四棵树挂得上、零未激活、零 FAILED），而且用的是**什么都没动过的**
 * 干净 profile——这与默认那台（播过种、钉过语言、挂过假模型端点的）判的不是同一件事，
 * 所以两套网关实现各留各的。
 */

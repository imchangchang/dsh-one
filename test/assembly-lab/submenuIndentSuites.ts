...
/**
 * 二级菜单父项右端的指示器（#172）：官方那对 chevron（收起 → 右向、展开 → 下向）。
 *
 * `name` 是**我们写进 `data-dshone-tree-icon` 的标记值**，即 `src/pure/officialIcons.ts` 表里的
 * 基名（官方 0.1.7-alpha.2 的叫法），两代 dsh 上都是这个字符串。
 *
 * `path` 是**渲染指纹**，取自官方 0.1.6-alpha.2 那代的定义（本机官方前端 `index-C04Zg7TP.js`
 * 里的 `IconChevronRightOutline14` / `IconChevronDownOutline14`：`width/height` 默认 14、
 * `viewBox="0 0 14 14"`、各一条 path）。官方 0.1.7-alpha.2 把这套图标重画了一遍（视框改成
 * `0 0 16 16`、改成描边画法，`path@d` 全变），**在那一版上跑这一套时形状那几条会红**——那是
 * 指纹钉在 0.1.6 的字节上，不是功能坏了（见 issue #236）。要两代都绿得把这两条改成「指纹
 * 属于哪一代的官方定义之一」的二选一判据，那是另一件事，别在没读 #236 的情况下顺手放宽。
 */
const CHEVRON = {
  collapsed: {

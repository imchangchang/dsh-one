...
interface BarFacts {
  ...
  gapAbove: number
  gapBelow: number
  /** 当页解析出来的纵向节奏（密度变量），缝隙要等于它。 */
  densityGroupGap: string
...
async function barFacts(page: OpenedPage['page']): Promise<BarFacts> {
  return page.evaluate((names: readonly string[]) => {
    ...
    const filter = document.querySelector('[data-dshone-tree="group-filter"]') as HTMLElement | null
    ...
    return {
      ...
      gapAbove: wrapRect.top - (rect(filter)?.bottom ?? wrapRect.top),
      gapBelow: (rect(list)?.top ?? wrapRect.bottom) - wrapRect.bottom,
      densityGroupGap: getComputedStyle(frame ?? document.body).getPropertyValue('--dsh-one-density-group-gap').trim(),
  ...
  run: async (ctx, check) => {
    ...
    try {
      ...
      for (const width of WIDTHS) {
        ...
        empty.push(facts)
        check.fact(
          `${width}px（未选）：计数「${facts.countText}」行数=${String(facts.countLines)} 盒宽=${facts.countWidth.toFixed(1)} 文字宽=${facts.countTextWidth.toFixed(1)}；上缝=${facts.gapAbove.toFixed(1)} 下缝=${facts.gapBelow.toFixed(1)}（密度档 ${facts.densityGroupGap}）；条=${
      ...
      for (const [index, width] of WIDTHS.entries()) {
        ...
        const expectedGap = Number.parseFloat(facts.densityGroupGap)
        check.ok(
          `${width}px：条与分组过滤条的缝隙 = 密度档纵向节奏（${facts.densityGroupGap}）`,
          Math.abs(facts.gapAbove - expectedGap) <= 1,
          `实测 ${facts.gapAbove.toFixed(1)}，期望 ${String(expectedGap)}`,
        )
        check.ok(
          `${width}px：条与列表首行的缝隙 = 密度档纵向节奏（${facts.densityGroupGap}）`,
          Math.abs(facts.gapBelow - expectedGap) <= 1,
          `实测 ${facts.gapBelow.toFixed(1)}，期望 ${String(expectedGap)}`,
        )
      }

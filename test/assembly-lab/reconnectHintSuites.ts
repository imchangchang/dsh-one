 ...
 * 是 `sidebar.settings` 座位上的贡献；chat 树没有官方侧栏壳、那个座位没人声明），
 ...
 * 等座位锚点零枚 ⇒ 官方 SettingsRoot 挂不上 ⇒ 官方提示永不出现）。改前/改后的整轮
 * 实测读数写在 #202 的汇报里。
 */
...
/**
 * `ui-settings-general` 注册的那些座位：chat 树**一个都没声明**，所以放行它之后
 * 这些锚点必须是零枚（挂不上 ≠ 渲染了空的），这就是「放行没把设置页面的东西带进来」
 * 的判据。座位名逐个取自官方 `dsh-client-ui-settings-general/lib/client.js` 的
 * `apply`：`sidebar.settings` 是它自己注册的外层座位，其余六处是它的 children。
 */
...
/** 页面上这些座位各有几枚锚点 + 提示行几枚（一次 evaluate 取全）。 */
...
export const RECONNECT_HINT_SUITE: LabSuite = {
  ...
  run: async (ctx, check) => {
    ...
    try {
      ...
      // ① 断线前：零行 + 官方那件进了 combo 却零座位锚点。
      const before = await seatAnchorCounts(page)
      const comboBefore = (await combosRequested(page)).join('\n')
      check.fact(`断线前读数：提示行=${String(before[HINT])} 座位锚点=${SETTINGS_SEATS.map((name) => `${name}=${String(before[name])}`).join(' ')}`)
      check.ok(
      ...
      check.eq(
        'chat：官方设置件注册的七处座位在这棵树上零声明（官方断线提示因此永不渲染，这一行只能我们自己出）',
        declaredSeats,
        [],
      )

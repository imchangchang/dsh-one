...
export const CHAT_BOOT_RACE_SUITE: LabSuite = {
  id: 'F-62',
  phase: 'new-feature',
  name: '对话面板启动注入：注入哪条会话，那条的正文就真的渲染出来（#205）',
  expect:
    '实验室自起的隔离实例 + 播种的真数据（不碰用户那台；会话名与文案都不写死，正文判据按目标会话自己那句正文认）。① **恢复键抢同一个选中值**（用户现场的形状）：一页 chat 树，官方恢复键（`dsh.sessions.current`）先写成**另一条**真会话，页面再带 `?session=<目标>` 注入目标——判「对话区里渲染的是**目标**那条的正文」（对话区正文里出现目标那条自己那句话、会话头写着它的标题：判的是用户看得见的那块区域，不是「外壳在」），且页面上**零**「目标没落定」的行（遮罩那句与按实际读数说的那句都不许有），`boot-timing` 里 `first-meta` 报的必须是目标会话（宿主拿得到这一页的身份）；同页零 pageerror、零崩溃、零装载未激活。② **真正的坏法（归档目标）**：目标会话只能在网关的归档名单里，页面不认它是当前会话——这一档如实记下对话区里**没有**它的正文（这正是用户看到的「只有外壳」），并判页面上说得出目标为什么没落定（`did not become current within …`，原因如实记成事实）；**同一套正文判据在这一档为假**，等于给①那条判据配了负向对照。③ **负向对照（不存在的 id）**：正文判据同样为假，且页面照样说得出原因——判据不是「外壳在就算过」。④ **运行时就地切换**（侧栏点会话行打开对话面板走的就是这条路）：同一页上按宿主的方式发出 `dshOne.switchSession`——先切到那条开不了的会话，页面必须说得出原因（改前这条路一个字都不说）；再切到一条正常会话，对话区必须换成它的正文（切换的既有行为没被这条通路带坏）；全程零 pageerror。',
  run: async (ctx, check) => {
    ...
    try {
      ...
      check.ok(
        '② 归档目标：对话区里**没有**它的正文（这就是用户看到的「只有外壳」——如实记下，不当它是对的）',

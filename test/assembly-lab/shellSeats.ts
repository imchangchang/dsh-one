...
/**
 * 三条常用理由（同一句话被多处引用，避免把同一条理由抄十遍而各自漂移）。
 */
const WHY = {
...
export const SEAT_CLICK_RULES: readonly SeatClickRule[] = [
  ...
  },
  {
    ...
    kind: 'waive',
    reason:
      '这一格是**整页**（对话区本体），条目的「第一枚可点元素」按位置点不安全：实测撞过一次——会话头右侧角那枚右栏开关与左端的 open-in-app 分裂按钮同在一片区域，测量与点击之间页面重渲过一次，点落到了**会经网关落到用户机器上**的那一枚上（`/open-in-app/open` 当场被 R-06 的原生副作用守卫拦下并判红，见本轮记录）。这一页的可点入口（composer 一排 / 对话区页签 / 右栏开关 / 助手动作 / 在访达中打开）都由本套件那份手工交互点表逐个覆盖，靠位置扫第一枚反而会把判据变成一个随机点',
  },
  {
    tree: 'chat',
    seat: 'rightbar',

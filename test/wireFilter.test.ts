...
test('block list 逐条复核（#202）：chat 树不再下线 ui-settings-general（它注册的座位那棵树一处都没声明）', () => {
  // 官方 ui-settings-general 的座位贡献全挂在 `sidebar.settings` 这个座位下（它的
  // `SettingsRoot`），而 `sidebar.settings` 只有官方**侧栏壳**声明——chat 树没有侧栏壳，
  // 所以放行它是整件停车、零渲染（#202 在实验室实测：放行前后元素集合 65 项逐项相同、
  // 设置座位锚点两边都是零枚，combo 里却真的多了它）。挂着它只是白背一个官方 id 依赖。
  assert.ok(!CHAT_BLOCKED_IDS.includes('@deepseek-ai/dsh-client-ui-settings-general'))
  // 同一件在 sidebar 树继续下线（那里座位声明得了，它的 SettingsRoot 会真注册进来）。
  assert.ok(SIDEBAR_BLOCKED_IDS.includes('@deepseek-ai/dsh-client-ui-settings-general'))
  ...
  // 它们注册的座位同样全在 `sidebar.settings` 之下，形状与本件一致——按同一条判据
